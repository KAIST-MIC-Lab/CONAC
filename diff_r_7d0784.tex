...
%DIF DEL tmp2.tex   Wed Jul 16 14:25:08 2025
%DIF ADD tmp1.tex   Wed Jul 16 14:25:08 2025
...
}\DIFaddend Before proceeding to the controller design, some mathematical preliminaries are revisited in Section \ref{sec:sub:math preliminaries}. 
...
\DIFaddbegin \begin{algorithm}[!t]
    ...
    \begin{algorithmic}[1]
        \STATE \textbf{\DIFadd{Initialize:}} \DIFadd{DNN weights $\estwth^{(0)}$, Lagrange multipliers $\lambda_j^{(0)}$
            ...
            \DIFaddbegin \STATE \textit{\DIFadd{// Neuro-adaptive control law (Section \ref{sec:ctrl design})}}
...
\DIFaddend \subsection{Mathematical Preliminaries}\label{sec:sub:math preliminaries}
...
\DIFadd{The DNN input vector ${\q}_n$ is defined as ${\q}_n:=(\q^\top,\dq^\top,\fe^\top,1)^\top$ to include the current state of the system, the filtered error, and the augmentation term $1$ to account for bias term in the input weight matrix $\wV_0$.
...
\DIFadd{The }\DIFaddend adaptation gain was set to $\alpha = 0.2$, and the filtering matrix in \eqref{eq:err:filtered} was chosen as $\mm{\Lambda}=\mydiag(5,15)$.
