...
However, since DNNs offer greater expressive power, making them more effective for complex system approximations \cite{Rolnick:2018aa}, variations of DNNs, such as long short-term memory (LSTM) networks for time-varying dynamics \cite{Griffis:2023aa}, physics-informed NNs (PINNs) for leveraging physical system knowledge \cite{Hart:2024aa}, and convolutional NNs (CNNs) for learning from historical sensor data \cite{Ryu:2024ac}, have further extended the capabilities of NAC systems.
...
This study extends our preliminary work in \cite{Ryu:2025aa}, which introduced inequality constraints on the weight and input norms within a constrained optimization framework for neuro-adaptive control (NAC) systems using a SHLNN. 
...
Section \ref{sec:ctrl design} introduces the proposed CONAC and the architecture of the DNN used in the controller. 
...
The DNN input vector ${\q}_n$ is defined as ${\q}_n=(\q^\top,\dq^\top,\fe^\top,1)^\top\in\R^{3n+1}$ to include the current state of the system, the filtered error, and the augmentation term $1$ to account for the bias term in the input weight matrix $\wV_0$.
...
% \ctxt{red}
% {
    In selecting the nonlinear function $\sigma(\cdot)$, the ReLU function \cite{Maas:2013aa} is one of the most widely used choices for large DNNs, since it effectively mitigates the gradient vanishing problem during error backpropagation. 
...
Similarly, let $\estNN_i$ denote the output of the $i$th layer with the estimated weight vector $\estwth$, and define $\estact_i=\act_i(\estNN_{i-1})$ and $\estact_i'= \ddtfrac{\estact_i}{\estNN_{i-1}}$.
...
\begin{algorithm}[!t]
    ...
    \begin{algorithmic}[1]
        ...
        \FOR{$k = 0 $ to $\infty$}
            ...
            \STATE $\fe^{(k)} \leftarrow \ddtt{\mv e^{(k)}}+\mm\Lambda \mv e^{(k)}$ based on \eqref{eq:err:filtered}
            ...
            \STATE \textit{// Neuro-adaptive control law (Section \ref{sec:ctrl design})}
                ...
                \STATE $\rbu^{(k)}\leftarrow\NN({\q}_n^{(k)};\estwth^{(k)}) $ based on \eqref{eq:control:est}
                \STATE Apply $\rbu^{(k)}$ to the system in \eqref{eq:sys1}

            \STATE \textit{// Train DNN (Section \ref{sec:adap_laws})}

            \STATE Compute $\pptfrac{\NN}{\estwth^{(k)}}$ as defined in \eqref{eq:gradient:phi}, $c_j$ and $\pptfrac{c_j}{\estwth^{(k)}}$ for all $j\in\mathcal{I}$          
            ...
            \lambda_j^{(k)}
            +
            \beta_j c_j\cdot T_s$ for all $j\in\mathcal{I}$\,based\,on\,\eqref{eq:adap:lbd} 
            \STATE $\lambda_j^{(k+1)} \leftarrow \max(\lambda_j^{(k+1)},0)$ for all $j\in\mathcal{I}$ from \eqref{eq:adap:lbd:max} 
...
In this section, the constraint-handling capability of all controllers is investigated.
...
% beta
The input constraint handling from $29.4$ s to $31.9$ s is examined in detail. 
...
This was attributed to the fact that the backpropagated error was scaled by the gradients of the activation function, which are bounded in $(0,1]$, \ie $\sigma(\cdot)=\tanh(\cdot)$.
