...
Invoking the fact that the amplitude of the control input depends only on $\estwth_k$, \ie see, \eqref{eq:control:est}, \eqref{eq:DNN:architecture} and $\norm{\act_k}<\infty$, the boundedness of inner layers' weights follows once the boundedness of $\estwth_k$ and $\fe$ is established.
...
For the weight constraint $c_{\theta_k}$, if the corresponding Lagrange multiplier $\lambda_{\theta_k}$ increases infinitely due to the constraint violation, $\estwth_k$ approaches to the origin, as the invariant set $\Theta_{\estwth_k}^{1}$ shrinks to a point.
The remaining control input constraints can be validated implicitly through the term ${\gamma}_2$ in \eqref{eq:lya2:dot}.
Similarly to $c_{\theta_k}$, when a control input constraint $c_j,\ \forall j\in\mathcal{I}\setminus \{\wth_i\}_{i\in\{0,\cdots,k\}}$ is violated, the corresponding Lagrange multiplier $\lambda_j$ increases infinitely, leading to increase in ${\gamma}_2$.
...
% Controllers: CONAC 
% ------------------------------------
\subsubsection*{Controller 1: \normalfont{(C$_1$)} \textit{CONAC with large $\beta_j$}}
...
% ------------------------------------
\subsubsection*{Controller 2: \normalfont{(C$_2$)} \textit{CONAC with small $\beta_j$}}
