...
However, to the best of the authors' knowledge, no prior work has applied constrained optimization to neuro-adaptive control systems with real-time weight adaptation. 
...
\begin{itemize}
    ...
    \item Since the proposed CONAC approximates the entire ideal control law without prior knowledge of the system dynamics, the system identification process for nominal conventional controller design can be avoided, which is often difficult and time-consuming to implement in practice. 
...
When a constraint $c_j$ becomes active, \ie $c_j > 0$, the corresponding Lagrange multiplier $\lambda_j$ increases to address the violation, see \eqref{eq:adap:lbd}.
...
First, weight norm constraints $[c_{\theta_i}(\estwth)]_{i\in\{0,\cdots ,k\}}\in\mathbb R^{k+1}$ defined in \eqref{eq:cstr:weight:ball}, are essential to prevent the weights from diverging during the adaptation process.
\begin{equation}
    c_{\theta_i}
    =
    \tfrac{1}{2}
...
Moreover, the Lagrange multiplier $\lambda_j$ does not diverge since the input constraints are satisfied before divergence can occur.
...
For the input constraints, large update rates were used to examine their effect: $\beta_{\overline{\tau}2} = \beta{\underline{\tau}_2} = 100$ and $\beta_{\rbu} = 10$, in order to investigate the effect of large update rates.
...
Furthermore, once the control saturation was lifted—typically due to changes in the desired trajectory—this resulted in oscillatory behavior, as CONAC's weights had been adapted to generate excessively high control inputs, see Fig.~\ref{fig:ctrl:result:tau:1}–\ref{fig:ctrl:result:tau:norm}, where the input constraints were lifted at $25.9$ s and $31.9$ s.
...
This was attributed to the fact that the backpropagated error being scaled by the activation function's gradient, which is bounded in $(0,1]$ (see \eqref{eq:NN:grad}).
...
This paper proposed a constrained optimization-based neuro-adaptive controller (CONAC) for unknown Euler-Lagrange systems, addressing both weight norm and input constraints within a unified optimization framework. 
