...
\begin{itemize}
    ...
    \item For $\mm{A}\in\R^{n\times m}$, $\myvec(\mm A):=(\myrow_1(\mm A^\top),\cdots,\myrow_m(\mm A^\top)  )^\top\in\R^{nm}$ denotes the vectorization of $\mm A$.
...
A critical aspect of neuro-adaptive control is the weight adaptation law, which governs how NN weights are updated. 
...
The tracking performances of CONAC with control input constraints, \ie (C$_1$), and (C$_2$), outperformed CONAC with the auxiliary system (C$_4$) in the second episodes, as shown in Table~\ref{tab:sim:L2}.
...
\KHCH{
        여기부터 문단 끝날때까지의 표현이 명료하지 않습니다. C3의 tracking performance가 좋은 것은 지표상으로 명확히 맞는 말이기 때문에 마치 tracking performance가 실제로 좋은 것은 아니다라는 뉘앙스의 논리 전개가 적절치 않습니다.

    Tracking performanc는 더 좋지만 과도한 control input을 사용하고, saturation이 지난 이후에는 진동 문제가 있다는 것만 명료하게 표현해주세요.
}
...
\KHCH{어색한 표현; 수정 요}\MSRY{필요없는 내용으로 판단하여 삭제했습니다.}
However, during the episodes, (C$_3$) increases the control input excessively large to reduce the tracking error, even though it can not be regulated due to the actuator limits as shown in Fig.~\ref{fig:ctrl:result:tau:2} and Fig.~\ref{fig:ctrl:result:tau:norm}, \ie the violations of the control input saturations of (C$_3$) grow larger in every saturated time interval.
...
\begin{figure}[!t]
	...
	\caption{
		...
		The notation ($\cdot'$) and ($\cdot''$) represent two different cases of large and small Lagrange multiplier, respectively, i.e., $\lambda_{j}' > \lambda_{j}''$.
	}
	% \vspace{1mm}
...
After the saturation was resolved at 31 seconds (see Fig.~\ref{fig:ctrl:result:tau:norm}), the auxiliary state $\zeta_1$ converged to zero, by the stable state matrix $\mm{A}_\zeta$ in the auxiliary system, \ie $\ddtt\mv{\zeta} = \mm{A}_\zeta \mv{\zeta} + \mm{B}_\zeta \Delta\rbu$ with $\Delta\rbu=\mv{0}_{2\times1}$, as described in Fig.~\ref{fig:ctrl:result:scope:aux}.
...
It is noteworthy that the constraint $c_j$ which satisfies Assumptions \ref{assum:convex} and \ref{assum:LICQ}, is convex in $\estwth_2$-space (see Lemma. \ref{lem:convex:angle}).
The large and small Lagrange multipliers can be interpreted as the case of (C$_1$) and (C$_2$), respectively, since the update rates of the Lagrange multipliers were set to $10^2$ times larger for (C$_1$) than (C$_2$).
...
With the larger $\lambda_j'$, the adaptation of $\estwth_2$ is more assertive than that with smaller $\lambda_j''$.
...
Besides, with small $\lambda_{\rbu}$, the adaptation direction of $\estwth_2$ is adjusted more conservatively, which leads to a slower satisfaction of the constraint $c_{\rbu}$, as shown in Fig.~\ref{fig:ctrl:result:scope:control} and  positive Lagrange multiplier $\lambda_{\rbu}$, as shown in Fig.~\ref{fig:ctrl:result:scope:beta}.
This allows (C$_2$) to show a smoother control input.
...
\KHCH{
    이부분은 "worth to note"하지는 않아보입니다. 그리고 FIg 5에서 이 것을 확인하기도 어렵습니다. 굳이 이내용을 유지하려면 Fig 6에서 multiplier가 발생했다 사리지는 현상을 표현할 때 사용해주세요.
}
\MSRY{
    부드럽게 제약조건만족이 안되는 것에 대한 변명(?)으로 포함하고 싶었습니다. 
    그렇기에 제약조건 파트 제일 마직막으로 뺐습니다.
}
In addition, it is worth to note that the control input constraints were repeatedly violated even after the constraint handling, as shown in Fig.~\ref{fig:ctrl:result:scope:control}.
This is because the controllers were implemented in discrete time with a low sampling rate of $250$ Hz.
...
However, as the weights and Lagrange multipliers were almost constant when the constraints were violated, the satisfaction of the KKT conditions can be implicitly shown, since the Lagrange multipliers of the active constraints were positive and those of inactive constraints were zero.
...
The computational times shown in Fig.~\ref{fig:ctrl:real:result:cmp:time}, indicates that the proposed CONAC can be implemented in real-time applications with a sampling rate of $250$ Hz, since the computational time of all controllers was less than $4$ ms.
