...
\KHCH{Abstract에 있는 것처럼 이 내용도 하나 추가해두죠.

The controller’s stability is rigorously
analyzed using Lyapunov theory, guaranteeing bounded
tracking errors and DNN weights.}
...
\begin{figure}[!t]
    ...
    \caption{Architecture of the constrained optimization-based neuro-adaptive controller (CONAC). \MSRY{SAT은 좀더 명확히 하기 위해서 사용하였음; 그렇지 않으면 아래 실험 피규어를 통해 포화를 넘어선 제어가 이ㅔㅂ력된다고 생각할까봐 그렇습니다; ddt의 경우 수식과 통일하기 위함이 맞습니다.}}
...
\KHCH{control direction과 sign of control input이 서로 다른 의미를 가지고 있지만 독자가 헷갈려할 수도 있을 것 같습니다. 이 부분 고민해주세요.}
...
\begin{assum}
    The constraint functions $c_j(\estwth),\ \forall j\in\mathcal I$, are convex in the $\tau$-space and satisfy $c_j(0) \le 0$ \color{red} and $c_j(\idealwth)\le 0$ \color{black}. \MSRY{desired controller가 제약조건 밖으로 나갈 수 있기 때문에 적절하지 않을 수 있음}
    \label{assum:convex}
\end{assum}
...
\MSRY{Explain with active set, later}
...
For the weight constraint $c_{\theta_k}$, if the corresponding Lagrange multiplier $\lambda_{\theta_k}$ increases sufficiently large \MSRY{본래 infinitely였습니다만 무한까지 가는 것을 가정하지 않기 때문에 충분히 커질때로 바꿈} due to the constraint violation, $\estwth_k$ approaches to the origin, as the invariant set $\Theta_{\estwth_k}^{1}$ shrinks to a point.
...
\begin{figure}[t]
    \centering
    \subfloat[Two-link manipulator model.\MSRY{ADD inertia}]{
...
% Controllers: CONAC 
% ------------------------------------
\subsubsection*{Controller 1: \normalfont{(C$_1$)} \textit{CONAC with large update rate of Lagrange Multipliers, $\beta_j,\forall j\in\mathcal{I}$}}
...
% ------------------------------------
\subsubsection*{Controller 2: \normalfont{(C$_2$)} \textit{CONAC with small update rate of Lagrange Multipliers, $\beta_j,\forall j\in\mathcal{I}$}}
...
\MSRY{사실, 제약조건이 부여된 것은 아니기 떄문에 위와 같이 표현하였음}
...
\KHCH{
    문단이 misleading 할 수 있습니다.
C3가 C4보다는 낫다고 되어 있는데 마지막에는 C3가 poor 하다고 합니다. 명료한 결론으로 바꿔주세요.
명확하지 않게 C4처럼 constraint를 고려하면 고려안하는 C3보다 더 안 좋아질 수 있다 등의 논리가 필요해보입니다.   
}
...
First, for (C$_4$), the auxiliary state $\zeta_1$ generated by constraint violation of $c_{\overline\tau_1}$. \KHCH{FIg6c를 언급하며 설명되면 좋겠습니다}
...
\KHCH{여기 아래 부터는, beta의 영향에 대해 설명하는데 lambda가 크고 작음만 언급됩니다. 명료하게 설명 수정해주세요.}
...
\MSRY{이 부분도 IECON과 마찬가지로 이산으로 돌아감으로 바로 보여주기는 힘들 수 있을 것 같습니다. 실제로 보시면 많은 진동이 보여서 정상상태의 KKT를 주장할 수 있을지 의문입니다.}
