...
\begin{prop} 
    The Coriolis/centripetal\MSRY{인용 책 그대로 적은 것입니다.} matrix $\rbVm$ can always be selected\MSRY{상동.} so that the matrix $\ddtt{\rbM}-2\rbVm$ is skew-symmetric, \ie $\mv{x}^\top(\ddtt{\rbM}-2\rbVm)\mv{x}=0,\ \forall \mv{x}\in\R^n$.
    \label{prop:skew}
\end{prop}
...
\begin{figure}[!t]
    ...
    \caption{Architecture of the constrained optimization-based neuro-adaptive controller (CONAC). \MSRY{SAT은 좀더 명확히 하기 위해서 사용하였음; 그렇지 않으면 아래 실험 피규어를 통해 포화를 넘어선 제어가 입력된다고 생각할까봐 그렇습니다; ddt의 경우 수식과 통일하기 위함이 맞습니다.}}
...
\begin{remark}
    Assumption \ref{assum:convex} is not restrictive, since the saturation functions are convex\KHCH{앞에 convex라는 표현이 나오지 않았는데 확인부탁합니다.}\MSRY{컨벡스란 말이 없지만 practical하게 대부분의 포화 함수는 컨벡스라는 말을 의도하였습니다.} in practice including the origin.
...
As a\MSRY{a?} result of the time derivative of $V_1$ in \eqref{eq:lya1:dot2}, the invariant set $\Theta_{\fe}^{1}$ of $\fe$ (\ie if $\fe$ leaves $\Theta_{\fe}^{1}$, $\ddtt V_1$ is negative) can be obtained using Lemma \ref{lem:stable:set} as follows:
\begin{equation}
    \Theta_{\fe}^{1} 
    = 
    \left\{ 
...
For the weight norm constraint $c_{\theta_k}$, if the corresponding Lagrange multiplier $\lambda_{\theta_k}$ \color{red}increases sufficiently large\color{black} \MSRY{본래 infinitely였습니다만 무한까지 가는 것을 가정하지 않기 때문에 충분히 커질때로 바꿈} due to the constraint violation, $\estwth_k$ approaches to the origin, as the invariant set $\Theta_{\estwth_k}^{1}$ shrinks to a point.
The remaining control input constraints can be validated implicitly through the term ${\gamma}_2$ in \eqref{eq:lya2:dot}.
Similarly to $c_{\theta_k}$, when a control input constraint $c_j,\ \forall j\in\mathcal{I}\setminus \{\wth_i\}_{i\in\{0,\cdots,k\}}$ is violated, the corresponding Lagrange multiplier $\lambda_j$ increases sufficiently large \MSRY{위와 같은 이유로 변경}, leading to increase in ${\gamma}_2$.
...
\KHCH{
    (30a)의 의미를 다음과 같이 바꾸는게 명확하겠습니다. 
    These sets, (30a) and (30b), can be physically interpreted as the current limitation imposed by the power source and the torque limitation of the second joint actuator, respectively.
}
\MSRY{
    전압 제한이 곧 전류 제한인 것인가요?
    전류 제한이라 하면 상태에 대한 제약조건이 부여된 것인가라는 오해가 생길가 걱정입니다.
    }
...
The controller effectively incorporated both convex input constraints and weight norm constraints, ensuring that actuator limitations and neural network parameters\MSRY{Weight?} remained within predefined bounds. 
