...
% ------------------------------------
The desired trajectory $\qd=\left({q_d}_1,{q_d}_2\right)^\top $ was generated using quintic (5$\textsuperscript{th}$-order) polynomial functions $\mathcal{P}^5(t;\mv{x}_1,\mv{x}_2,t_f)$, which satisfy the initial and final joint angles $\mv{x}_1\in\R^2$ and $\mv{x}_2\in\R^2$, as well as zero initial and final joint velocities. 
