...
\color{red}
    Even though the DNNs have a higher approximation capability than the SHLNNs \cite{Rolnick:2018aa}, here, the SHLNN is utilized for simplicity and low computational complexity.
\color{black}
...
All controllers had the same control parameters except their crucial parameters (\ie $\lambda$, $\rho$ and $\beta_j$) as $k_q=1.1$, $k_z=10$, $\widehat{\mm M}=\mm I_2$ and $\mm \Lambda=\text{diag}([5,1,15,15])$.
