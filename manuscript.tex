...
The algorithm of the proposed CONAC is presented by Algorithm \ref{alg:CONAC}, implemented in a discrete-time controller with a sampling time of~$T_s$.
...
\begin{algorithm}[!t]
    ...
    \begin{algorithmic}[1]
        \STATE \textbf{Initialize:} DNN weights $\estwth^0$ and Lagrange multipliers $\lambda_j^0$
        \FOR{$k = 0 $ to $\infty$}
...
The DNN input vector ${\q}_n$ is defined as ${\q}_n:=(\q^\top,\dq^\top,\fe^\top,1)^\top$ to include the current state of the system, the filtered error, and the augmentation term $1$ for the bias term of the DNN.
