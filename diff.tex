...
Similarly to $c_{\theta_k}$, when an input constraint $c_j,\ \forall j\in\mathcal{I}\setminus \{\theta_i\}_{i\in\{0,\cdots,k\}}$, is violated, the corresponding Lagrange multiplier $\lambda_j$ increases sufficiently \DIFdelbegin \DIFdel{large}\DIFdelend \DIFaddbegin \DIFadd{to a large value}\DIFaddend , leading to an increase in ${\gamma}_2$.
...
\begin{equation}
    ...
    .
\end{equation}
The boundedness of $\estwth_k$ \DIFdelbegin \DIFdel{also can }\DIFdelend \DIFaddbegin \DIFadd{can also }\DIFaddend be ensured by the invariant set $\Theta_{\theta_k}^{2}=\{ \estwth_k \in \R^{\Xi_{k}} \mid \norm{\estwth_k} \le \overline\theta_k \}$, as $\lambda_{\theta_k}$ remains positive unless the weight norm constraint $c_{\theta_k}$ is satisfied.
...
The two-link robotic manipulator \DIFaddbegin \DIFadd{was }\DIFaddend actuated by two Cubemars AK10-90 servo motors. 
The servos were controlled using the OpenCR1.0 board\DIFaddbegin \DIFadd{, }\DIFaddend which transmits the torque reference to the corresponding servo via controller area network (CAN) communication.
The system dynamics of the two-link robotic manipulator can follow the Euler-Lagrange equation \eqref{eq:sys1}, where $\q$ denotes \DIFaddbegin \DIFadd{the }\DIFaddend joint angles and $\rbu$ denotes the control input (joint torques), and is illustrated in Fig.~\ref{fig:robot:model}. 
...
Therefore, \DIFdelbegin \DIFdel{in order }\DIFdelend to avoid the instability caused by the initial poor performance of CONAC, a warm-up phase was introduced \DIFdelbegin \DIFdel{, }\DIFdelend prior to applying the desired trajectory.
...
Therefore, controller (C\DIFdelbegin \DIFdel{$1$}\DIFdelend \DIFaddbegin \DIFadd{$_1$}\DIFaddend ) utilizes the full capability of the actuator, see \eqref{eq:sat:tau2} and \eqref{eq:sat:norm}.
The weight norm constraints $c_{\theta_i}(\cdot),\ \forall i\in\{0,\cdots,k\}$, defined in \eqref{eq:cstr:weight:ball}, were also incorporated into \DIFaddbegin \DIFadd{the }\DIFaddend CONAC framework with $\overline\theta_0=\overline\theta_1=\overline\theta_2=6$.
...
For the input constraints, large update rates were used: \DIFdelbegin \DIFdel{$\beta_{\overline{\tau}2} = \beta{\underline{\tau}_2} = 100$ }\DIFdelend \DIFaddbegin \DIFadd{$\beta_{\overline{\tau}_2} = \beta_{\underline{\tau}_2} = 100$ }\DIFaddend and $\beta_{\rbu} = 10$, \DIFdelbegin \DIFdel{in order }\DIFdelend to investigate the effect of large update rates.
...
The third controller was identical to (C\DIFdelbegin \DIFdel{$1$}\DIFdelend \DIFaddbegin \DIFadd{$_1$}\DIFaddend ) and (C\DIFdelbegin \DIFdel{$2$}\DIFdelend \DIFaddbegin \DIFadd{$_2$}\DIFaddend ), except that the input constraints were removed to highlight the \DIFdelbegin \DIFdel{constraint handling }\DIFdelend \DIFaddbegin \DIFadd{constraint-handling }\DIFaddend capability of the proposed CONAC.
...
All other properties and parameters were kept the same as in (C$_1$), (C$_2$)\DIFaddbegin \DIFadd{, }\DIFaddend and (C$_3$)\DIFdelbegin \DIFdel{expect }\DIFdelend \DIFaddbegin \DIFadd{, except }\DIFaddend that the weight boundedness was handled using the projection operator used in \cite{Patil:2022aa}.
...
Furthermore, the stable learning observed under random weight initialization and without prior knowledge of the system \DIFdelbegin \DIFdel{, }\DIFdelend demonstrated the effectiveness of the proposed CONAC in enabling online learning capability.
...
In this section, the \DIFdelbegin \DIFdel{constraint handling }\DIFdelend \DIFaddbegin \DIFadd{constraint-handling }\DIFaddend capability of all controllers \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend investigated.
...
% beta
The input constraint handling \DIFdelbegin \DIFdel{between }\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend $29.4$ s to $31.9$ s is examined in detail. 
...
This was attributed to the fact that the backpropagated error was scaled by the gradients of the activation function, which \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend bounded in $(0,1]$, \ie $\sigma(\cdot)=\tanh(\cdot)$.
...
The gradient of $c_{\rbu}$ with respect to $\estwth$ \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend given by
\begin{equation}
