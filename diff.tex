...
%DIF DEL tmp2.tex   Mon Jun 23 00:18:32 2025
%DIF ADD tmp1.tex   Mon Jun 23 00:18:32 2025
...
More recently, neuro-adaptive control approaches have been introduced to approximate uncertain or unknown system dynamics or entire control laws using neural networks (NNs) \cite{Lewis:1998aa,Farrell:2006aa}. 
...
This issue becomes more pronounced in neuro-adaptive control architectures where NNs are used to augment conventional model-based controllers—such as feedback linearization or backstepping—rather than acting as the sole control input. 
...
\DIFaddbegin \MSRY{각 문단에 이미 문제점을 지적해서 요약을 안해도 괜찮을 것 같습니다.}
\ctxt{red}
{
    \DIFaddend Although both the existing methods for weight boundedness and input constraints have shown effectiveness, they come with significant limitations. 
...
A promising approach to overcoming these limitations lies in constrained optimization.
...
However, to the best of the authors' knowledge, no prior work has applied constrained optimization to \DIFdelbegin \DIFdel{adaptive }\DIFdelend \DIFaddbegin \DIFadd{neuro-adaptive }\DIFaddend control systems with real-time weight adaptation. 
...
\begin{itemize}
    ...
    \item Since the proposed CONAC approximates the entire ideal control law \DIFdelbegin \DIFdel{with no }\DIFdelend \DIFaddbegin \DIFadd{without }\DIFaddend prior knowledge of the system dynamics, the system identification process for nominal conventional controller design can be avoided, which is often difficult and time-consuming to implement in practice. 
...
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.9\linewidth]{src/figures/Controller.drawio.pdf}
...
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.9\linewidth]{src/figures/DNN.drawio.pdf}
    \DIFaddendFL \caption{
...
\DIFaddbegin \MSRY{생략 가능할 것 같습니다.}
\ctxt{red}
{
    \DIFaddend In selecting the nonlinear function $\sigma(\cdot)$, the ReLU function \cite{Maas:2013aa} is one of most widely used choices for large DNNs, since it effectively mitigates the gradient vanishing problem during error backpropagation. 
...
\DIFaddbegin }
\DIFaddend In this study, the hyperbolic tangent function $\tanh(\cdot)$ was selected as the nonlinear function, \ie $\sigma(x) = \tanh(x),\ \forall x\in\R$, which provides desirable boundedness with $\norm{\sigma(x)}<1$ and \DIFdelbegin \DIFdel{$\norm{\ddtfrac{\sigma(x)}{x}}\le 1$}\DIFdelend \DIFaddbegin \DIFadd{$0<\norm{\ddtfrac{\sigma(x)}{x}}\le 1$}\DIFaddend .
...
When a constraint $c_j$ becomes active, \ie $c_j > 0$, the corresponding Lagrange multiplier $\lambda_j$ increases to address the violation, \DIFdelbegin %DIFDELCMD < \ie %%%
\DIFdelend see \eqref{eq:adap:lbd}.
...
\ctxt{red}
{
    \DIFaddend Note that this adaption law is similar to the ALM in \cite{Nocedal:2006aa}, where the adaptation law for Lagrange multipliers is given by $\lambda_j\leftarrow \max\left(\lambda_j+\tfrac{c_j}{\mu},0\right)$, with $\mu\in\R_{>0}$ being the penalty parameter. 
\DIFaddbegin }
\DIFaddend 
...
First, weight norm constraints \DIFdelbegin \DIFdel{$\mv{c}_{\theta}(\estwth):= [c_{\theta_i}(\estwth)]_{i\in\{0,\cdots ,k\}}\in\mathbb R^{k+1}$ }\DIFdelend \DIFaddbegin \DIFadd{$[c_{\theta_i}(\estwth)]_{i\in\{0,\cdots ,k\}}\in\mathbb R^{k+1}$ }\DIFaddend defined in \eqref{eq:cstr:weight:ball}, are essential to prevent the weights from diverging during the adaptation process.
\begin{equation}
    c_{\theta_i}
    =
    \tfrac{1}{2}
...
\begin{theorem}
    For the dynamical system described in \eqref{eq:sys1}, the neuro-adaptive controller in \eqref{eq:control:est} with the weight adaptation laws in \eqref{eq:adaptation law} ensure the boundedness of the filtered error $\fe$ and the weight estimate $\estwth$, under the \DIFdelbegin \DIFdel{control }\DIFdelend input constraints satisfying Assumption \ref{assum:convex} and \ref{assum:LICQ}.
    \ctxt{red}{
...
Moreover, the Lagrange multiplier $\lambda_j$ does not diverge since the \DIFdelbegin \DIFdel{control }\DIFdelend input constraints are satisfied before divergence can occur.
...
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.9\linewidth]
        \DIFaddendFL % \includegraphics[width=\figSizeOneCol\linewidth]
        {
...
\DIFdelendFL input constraints.& \DIFaddbeginFL \multirow{2}{*}{ \shortstack[l]{No input constraints\\are imposed.} } \DIFaddendFL \\
    ...
    \hline
        \multirow{2}{*}{(C$_4$)}& NAC with & $\overline{\tau}_1=-\underline{\tau}_1=10.428$ \\
        & auxiliary system. & $\overline{\tau_2}=-\underline{\tau_2}=3.5$. \\    
    \hline
    \end{tabular}
...
For the \DIFdelbegin \DIFdel{control }\DIFdelend input constraints, large update rates were used to examine their effect: \DIFdelbegin \DIFdel{$\beta_{\overline{\tau}2} = \beta{\underline{\tau}_2} = 10^2$ }\DIFdelend \DIFaddbegin \DIFadd{$\beta_{\overline{\tau}2} = \beta{\underline{\tau}_2} = 100$ }\DIFaddend and $\beta_{\rbu} = 10$, in order to investigate the effect of large update rates.
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{see Fig.~\ref{fig:ctrl:result:tau:1}–\ref{fig:ctrl:result:tau:norm}, where the input constraints were lifted at $25.9$ s and $31.9$ s.
}\DIFaddend 
...
\DIFaddbegin \ctxt{red}
{
    \DIFaddend Under Assumptions \ref{assum:convex} and \ref{assum:LICQ}, the convexity of $c_j$ in the $\estwth_2$-space (see Lemma~\ref{lem:convex:angle}) further supports this adaptive behavior.
\DIFaddbegin }
\MSRY{이거 위로 올라가야 할까요?}
\DIFaddend 
...
This \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend attributed to the fact that the backpropagated error being scaled by the activation function's gradient, which is bounded \DIFdelbegin \DIFdel{between $-1$ and $1$ }\DIFdelend \DIFaddbegin \DIFadd{in $(0,1]$ }\DIFaddend (see \eqref{eq:NN:grad}).
...
\ctxt{red}
{
    \DIFaddend However, the near-constant values of the weights and Lagrange multipliers during constraint violation suggest an implicit satisfaction of the KKT conditions, as shown in Fig.~\ref{fig:ctrl:result:weight} and Fig.~\ref{fig:ctrl:result:scope:beta}
    \DIFdelbegin \DIFdel{.
}\DIFdelend Specifically, the Lagrange multipliers corresponding to active constraints were positive, while those associated with inactive constraints remained zero.
\DIFaddbegin }
\ctxt{blue}
{
    \DIFadd{This is because, the KKT conditions are typically defined for continuous-time systems, and their direct application to discrete-time systems requires careful consideration of the sampling rate and the dynamics of the system.
    Although exact satisfaction of the KKT conditions was not observed—due to the lack of a clear steady state in the discrete-time implementation—the control results remain sufficiently close to the optimal behavior implied by the KKT conditions.
}}
\DIFaddend 
...
This paper proposed a constrained optimization-based neuro-adaptive controller (CONAC) for unknown Euler-Lagrange systems, addressing both weight norm and input constraints within a rigorous optimization framework. 
The stability of CONAC was analyzed using Lyapunov theory, demonstrating bounded tracking and weight estimation errors under real-time adaptation.
