...
%DIF DEL tmp2.tex   Sun Jun 22 02:27:49 2025
%DIF ADD tmp1.tex   Sun Jun 22 02:27:49 2025
%DIF < \documentclass[journal]{IEEEtran}
%DIF -------
\documentclass[letterpaper, journal]{IEEEtran} %DIF > 
%DIF -------
...
\newcommand*{\figSizeOneCol}{0.98}
 %DIF > 
\newcommand{\ctxt}[2]{\color{#1}#2\color{black}} %DIF > 
...
\begin{IEEEkeywords}
    Neuro-adaptive control, constrained optimization, deep neural network, input constraint\DIFaddbegin \DIFadd{, weight constraint}\DIFaddend .
\end{IEEEkeywords}

\section*{Notation}
\DIFdelbegin \DIFdel{In this study, the following notation is used:
}\DIFdelend 

\DIFdelbegin %DIFDELCMD < \begin{itemize}
%DIFDELCMD <     \item %%%
\DIFdel{:= denotes ``is defined as" or ``is equal to".
    }%DIFDELCMD < \item %%%
\DIFdel{$\R$, $\R^n$ and $\R^{n\times m}$ denote the set of real numbers, $n\times 1$ dimensional vectors and $n\times m$ dimensional matrices, respectively, and $\R_{\ge0}:=[0,\infty)$ and $\R_{>0}:=(0,\infty)$.
    }%DIFDELCMD < \item %%%
\DIFdel{$(\cdot)^\top$ denotes the transpose of a vector or matrix.
    }%DIFDELCMD < \item %%%
\DIFdel{$\norm{\cdot}$ and $\norm{\cdot}_F$ denote the Euclidean and Frobenius norm, respectively.
    }%DIFDELCMD < \item %%%
\DIFdelend %DIF >  In this study, the following notation is used:
\DIFaddbegin 
...
\DIFadd{The Kronecker product is denoted by }\DIFaddend $\otimes$ \DIFdelbegin \DIFdel{denotes the Kronecker product }\DIFdelend \cite[Chap. 7, Def. 7.1.2]{Bernstein:2009aa}.
\DIFdelbegin %DIFDELCMD < \item %%%
\DIFdelend A vector and a matrix are denoted by $\mv{x}=[x_i]_{i\in\{1,\cdots,n\}}\in\R^n$ and $
\mm A
:= 
[a_{ij}]
_{
...
$, respectively.
\DIFdelbegin %DIFDELCMD < \item %%%
\DIFdelend \DIFaddbegin \DIFadd{Given a matrix $\mm A\in\R^{n\times m}$, }\DIFaddend $\myrow_i(\mm A)$ denotes the \DIFdelbegin \DIFdel{$i\textsuperscript{th}$ row of the matrix $\mm A\in\R^{n\times m}$. 
}%DIFDELCMD < \item %%%
\DIFdelend \DIFaddbegin \DIFadd{$i$th row of $\mm A$. 
}\DIFaddend For the matrix $\mm{A}\in\R^{n\times m}$, $\myvec(\mm A):=(\myrow_1(\mm A^\top),\cdots,\myrow_m(\mm A^\top) )^\top\in\R^{nm}$ denotes the vectorization of $\mm A$.
%DIF <  \item $\mycol_i(A)$ and $\myrow_j(A)$ denote the $i\textsuperscript{th}$ column and $j\textsuperscript{th}$ row of $A\in\R^{n\times m}$, respectively.
    %DIF <  \item $\myvec(A):= [\mycol_1(A)^\top  ,\cdots,\mycol_m(A)^\top  ]^\top   $ for $A\in\R^{n\times m}$.
    \DIFdelbegin %DIFDELCMD < \item %%%
\DIFdelend $\lambda_{\min}(\mm A)$ denotes the minimum eigenvalue of the matrix $\mm A\in\R^{n\times n}$.
\DIFdelbegin %DIFDELCMD < \item %%%
\DIFdel{$\mm I_n$ denotes the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend $n\times n$ identity \DIFdelbegin \DIFdel{matrix, and $\mm 0_{n\times m}$ denotes the }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend $n\times m$ zero \DIFdelbegin \DIFdel{matrix.
}%DIFDELCMD < \end{itemize}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{matrices are denoted by $\mm I_n$ and $\mm 0_{n\times m}$, respectively.
}\DIFaddend 
...
To address these challenges, adaptive control methods have been widely employed to ensure robust performance for uncertain \DIFdelbegin \DIFdel{(unknown ) }\DIFdelend \DIFaddbegin \DIFadd{or unknown }\DIFaddend systems \cite{Ioannou:2006aa, Tao:2003aa}.

More recently, neuro-adaptive control approaches have been introduced to approximate uncertain \DIFdelbegin \DIFdel{(unknown ) }\DIFdelend \DIFaddbegin \DIFadd{or unknown }\DIFaddend system dynamics or entire control laws using neural networks (NNs) \cite{Lewis:1998aa,Farrell:2006aa}. 
...
Conventionally, SHL and RBF NNs are often employed to approximate uncertain \DIFdelbegin \DIFdel{(unknown ) }\DIFdelend \DIFaddbegin \DIFadd{or unknown }\DIFaddend system dynamics or controllers due to their simplicity \cite{Esfandiari:2014aa,Esfandiari:2015aa,Yesildirek:1995aa,Gao:2006aa}.
...
This issue \DIFdelbegin \DIFdel{is exacerbated }\DIFdelend \DIFaddbegin \DIFadd{becomes more pronounced }\DIFaddend in neuro-adaptive \DIFdelbegin \DIFdel{controllers that utilize NNs as additional control inputs to compensate for uncertain system dynamics, while }\DIFdelend \DIFaddbegin \DIFadd{control architectures where NNs are used to augment conventional }\DIFaddend model-based \DIFdelbegin \DIFdel{control methods}\DIFdelend \DIFaddbegin \DIFadd{controllers}\DIFaddend —such as feedback linearization or backstepping—\DIFdelbegin \DIFdel{are employed}\DIFdelend \DIFaddbegin \DIFadd{rather than acting as the sole control input}\DIFaddend . 
In such \DIFdelbegin \DIFdel{architectures, when }\DIFdelend \DIFaddbegin \DIFadd{cases, if }\DIFaddend the system model is inaccurate or the dynamics \DIFdelbegin \DIFdel{that the NN is intended to learn are already stabilizing, the model-based control inputs may produce excessively aggressive control inputs.
This often lead to unnecessary saturation of the control inputs, as discussed in }\DIFdelend \DIFaddbegin \DIFadd{targeted by the NN are already contributing to stability, the combined control effort may result in overly aggressive inputs, increasing the risk of input saturation }\DIFaddend \cite{Khalil:2002aa}.

To address the aforementioned issue, several approaches have been proposed in \cite{Gao:2006aa,Arefinia:2020aa,He:2016aa,Peng:2020aa,Esfandiari:2014aa,Karason:1994aa,Esfandiari:2015aa} to handle input constraints in neuro-adaptive control systems.
...
On the other hands, in \cite{Esfandiari:2014aa,Karason:1994aa,Esfandiari:2015aa}, the auxiliary systems incorporated into the adaptation law by adding the auxiliary states to the feedback error for learning.
...
\DIFdelbegin \DIFdel{Moreover, from another perspective of learning , incorporating NNs with the }\DIFdelend \DIFaddbegin \DIFadd{From a learning perspective, augmenting }\DIFaddend conventional control methods \DIFdelbegin \DIFdel{by adding the NN output to the conventional control methods can disrupt }\DIFdelend \DIFaddbegin \DIFadd{with NNs by simply adding NN outputs to existing control inputs can interfere with }\DIFaddend the adaptation process\DIFdelbegin \DIFdel{of the NN. 
This is }\DIFdelend \DIFaddbegin \DIFadd{. 
This occurs }\DIFaddend because the adaptation law \DIFdelbegin \DIFdel{relies }\DIFdelend \DIFaddbegin \DIFadd{typically depends }\DIFaddend on the feedback error, which becomes \DIFdelbegin \DIFdel{decoupled from }\DIFdelend \DIFaddbegin \DIFadd{insensitive to }\DIFaddend the NN output \DIFdelbegin \DIFdel{due to the dominance of the conventional methods. 
In these cases}\DIFdelend \DIFaddbegin \DIFadd{when the conventional controller dominates the system behavior. 
As a result}\DIFaddend , the NN \DIFdelbegin \DIFdel{is forced to counteract }\DIFdelend \DIFaddbegin \DIFadd{ends up counteracting }\DIFaddend the conventional control \DIFdelbegin \DIFdel{inputs rather than learn }\DIFdelend \DIFaddbegin \DIFadd{effort rather than directly learning }\DIFaddend the system dynamics\DIFdelbegin \DIFdel{directly.
As a result, the NN's ability to learn and adaptis undermined.
Therefore, it is recommended that input constraints }\DIFdelend \DIFaddbegin \DIFadd{, thereby weakening its ability to adapt.
To address this, input constraints should }\DIFaddend be handled within a unified \DIFdelbegin \DIFdel{adaptation frameworklike \mbox{%DIFAUXCMD
\cite{Esfandiari:2014aa,Karason:1994aa,Esfandiari:2015aa}}\hskip0pt%DIFAUXCMD
, }\DIFdelend \DIFaddbegin \DIFadd{adaptive framework—such as those proposed in \mbox{%DIFAUXCMD
\cite{Esfandiari:2014aa,Karason:1994aa,Esfandiari:2015aa}}\hskip0pt%DIFAUXCMD
—}\DIFaddend where both adaptation and constraint satisfaction are \DIFdelbegin \DIFdel{addressed simultaneously, }\DIFdelend \DIFaddbegin \DIFadd{treated jointly }\DIFaddend without relying on \DIFdelbegin \DIFdel{the }\DIFdelend conventional control inputs.
...
\DIFdelbegin \DIFdel{To overcome these limitations , constrained optimizationoffers a promising approach}\DIFdelend \DIFaddbegin \DIFadd{A promising approach to overcoming these limitations lies in constrained optimization}\DIFaddend .
By formulating the neuro-adaptive control problem as an unified optimization problem with constraints, it is possible to adapt the NN weights while minimizing an objective function, \eg tracking error, subject to both weight and input constraints, simultaneously.
...
\begin{itemize}
    ...
    \item Since the proposed CONAC \DIFdelbegin \DIFdel{approximate }\DIFdelend \DIFaddbegin \DIFadd{approximates }\DIFaddend the entire ideal control law with no prior knowledge of the system dynamics, the system identification process for nominal conventional controller design can be avoided, which is often difficult and time-consuming to implement in practice. 
    ...
    \item The proposed CONAC's stability is rigorously analyzed using Lyapunov theory, ensuring that the tracking error and DNN weights remain bounded. This analysis guarantees that the proposed CONAC can achieve stable \DIFdelbegin \DIFdel{adaptation online }\DIFdelend \DIFaddbegin \DIFadd{online adaptation}\DIFaddend .
    \item The proposed CONAC is implemented in real-time on a 2-DOF robotic manipulator, demonstrating its effectiveness in achieving accurate trajectory tracking while satisfying all imposed constraints. The experimental results validate the theoretical findings and \DIFdelbegin \DIFdel{show }\DIFdelend \DIFaddbegin \DIFadd{confirm }\DIFaddend the practical applicability of the proposed CONAC.
\end{itemize}

\hfill

This study \DIFdelbegin \DIFdel{is extended from }\DIFdelend \DIFaddbegin \DIFadd{extends }\DIFaddend our preliminary work \DIFdelbegin \DIFdel{presented in \mbox{%DIFAUXCMD
\cite{Ryu:2024aa}}\hskip0pt%DIFAUXCMD
where the inequality constraint for weight norm was imposed on the neuro-adaptive control design to establish the weight boundedness using }\DIFdelend \DIFaddbegin \DIFadd{in \mbox{%DIFAUXCMD
\cite{Ryu:2024aa}}\hskip0pt%DIFAUXCMD
, where an inequality constraint on the weight norm was introduced within a }\DIFaddend constrained optimization framework \DIFdelbegin \DIFdel{.
Similar to weight constraints }\DIFdelend \DIFaddbegin \DIFadd{to ensure boundedness of the NN weights. 
In addition to the weight constraints considered }\DIFaddend in \cite{Ryu:2024aa}, \DIFaddbegin \DIFadd{the present work also incorporates }\DIFaddend control input constraints \DIFdelbegin \DIFdel{are also incorporated into the constrained optimization }\DIFdelend \DIFaddbegin \DIFadd{into the same }\DIFaddend framework to address the convex input saturation problem\DIFdelbegin \DIFdel{as well. 
Furthermore, }\DIFdelend \DIFaddbegin \DIFadd{. 
Moreover, while the previous study was limited to numerical simulations, }\DIFaddend the \DIFdelbegin \DIFdel{applicability of the }\DIFdelend proposed CONAC is \DIFdelbegin \DIFdel{investigated in a }\DIFdelend \DIFaddbegin \DIFadd{experimentally validated through }\DIFaddend real-time implementation on a 2-DOF robotic manipulator\DIFdelbegin \DIFdel{; while the referenced work was limited to the numerical simulation}\DIFdelend .

\subsection{Organization}

The remainder of this paper is organized as follows. 
Section \ref{sec:Problem Formulation} presents the target system and control objective.
Section \ref{sec:ctrl design} introduces the proposed CONAC and the architecture of DNN \DIFaddbegin \DIFadd{used }\DIFaddend in the controller. 
In Section \ref{sec:adap_laws}, the adaptation law is developed.
Section \ref{sec:stability} analyzes \DIFdelbegin \DIFdel{the stability of the proposed CONAC.
A }\DIFdelend \DIFaddbegin \DIFadd{a stability of CONAC.
Section \ref{sec:sim} presents a }\DIFaddend real-time implementation of \DIFdelbegin \DIFdel{the proposed CONAC is presented in Section \ref{sec:sim}, where the proposed CONAC is applied to }\DIFdelend \DIFaddbegin \DIFadd{CONAC on }\DIFaddend a 2-DOF robotic manipulator.
Finally, Section \ref{sec:conclusion} concludes the paper and discusses potential future work.
\DIFdelbegin \DIFdel{The candidates of }\DIFdelend \DIFaddbegin \DIFadd{Candidates for }\DIFaddend convex input constraints are \DIFdelbegin \DIFdel{presented }\DIFdelend \DIFaddbegin \DIFadd{provided }\DIFaddend in Appendix \ref{sec:appen:cstr}. 
...
This is because \DIFdelbegin \DIFdel{, the }\DIFdelend actuator limits are \DIFdelbegin \DIFdel{naturally convex, such as min/max torque limits, and bounds look like a box or ball }\DIFdelend \DIFaddbegin \DIFadd{typically convex—for example, minimum and maximum torque constraints—which define box- or ball-shaped bounds }\DIFaddend in the input space.
...
\DIFdelbegin \DIFdel{The algorithm of the proposed CONAC is given by Algorithm \ref{alg:CONAC}. 
}\DIFdelend %DIF >  The algorithm of the proposed CONAC is given by Algorithm \ref{alg:CONAC}. 
...
\begin{figure}[!t]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.999\linewidth]{src/figures/Controller.drawio.pdf}
%DIFDELCMD <     %%%
%DIF <  \includesvg[width=0.75\linewidth]{Controller.drawio.svg}
    \DIFdelendFL \DIFaddbeginFL \includegraphics[width=.8\linewidth]{src/figures/Controller.drawio.pdf}
    \DIFaddendFL \caption{Architecture of the proposed constrained optimization-based neuro-adaptive controller (CONAC).}
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Proposed CONAC Algorithm}}%DIFAUXCMD
%DIFDELCMD < \label{alg:CONAC}
%DIFDELCMD <     \begin{algorithmic}[1]
%DIFDELCMD <         \STATE %%%
\textbf{\DIFdel{Initialize:}} %DIFAUXCMD
\DIFdel{DNN weights $\estwth$ and Lagrange multipliers $\lambda_j$
        }%DIFDELCMD < \FOR{each sampling time}
%DIFDELCMD <             \STATE %%%
\DIFdel{Measure current state $\q,\dq$
            }%DIFDELCMD < \STATE %%%
\DIFdel{Compute filtered error $\fe$ \eqref{eq:err:filtered}
            }%DIFDELCMD < \STATE %%%
\DIFdel{Update DNN weights $\estwth$ using $\fe$ \eqref{eq:adap:th}
            }%DIFDELCMD < \STATE %%%
\DIFdel{Update Lagrange multipliers $\lambda_j$ based on constraint function $c_j$ \eqref{eq:adap:lbd} and \eqref{eq:adap:lbd:max}
            }%DIFDELCMD < \STATE %%%
\DIFdel{Compute control input $\rbu$ (DNN output) \eqref{eq:control:est} and \eqref{eq:DNN:architecture}
            }%DIFDELCMD < \STATE %%%
\DIFdel{Apply control input $\rbu$ to the system \eqref{eq:sys1}
        }%DIFDELCMD < \ENDFOR
%DIFDELCMD <     \end{algorithmic}
%DIFDELCMD < \end{algorithm}
%DIFDELCMD < %%%
\DIFdelend %DIF >  \begin{algorithm}[!t]
%DIF >      \caption{Proposed CONAC Algorithm}\label{alg:CONAC}
%DIF >      \begin{algorithmic}[1]
%DIF >          \STATE \textbf{Initialize:} DNN weights $\estwth$ and Lagrange multipliers $\lambda_j$
%DIF >          \FOR{each sampling time}
%DIF >              \STATE Measure current state $\q,\dq$
%DIF >              \STATE Compute filtered error $\fe$ \eqref{eq:err:filtered}
%DIF >              \STATE Update DNN weights $\estwth$ using $\fe$ \eqref{eq:adap:th}
%DIF >              \STATE Update Lagrange multipliers $\lambda_j$ based on constraint function $c_j$ \eqref{eq:adap:lbd} and \eqref{eq:adap:lbd:max}
%DIF >              \STATE Compute control input $\rbu$ (DNN output) \eqref{eq:control:est} and \eqref{eq:DNN:architecture}
%DIF >              \STATE Apply control input $\rbu$ to the system \eqref{eq:sys1}
%DIF >          \ENDFOR
%DIF >      \end{algorithmic}
%DIF >  \end{algorithm}
...
\begin{equation}
    ...
    \label{eq:lya1:dot1}
\end{equation} 
Therefore, invoking Lemma \ref{lem:stable:set}, it can be concluded that the filtered error $\fe$ is \DIFdelbegin \DIFdel{exponentially stable, }\DIFdelend \DIFaddbegin \DIFadd{uniformly ultimately bounded }\DIFaddend with $\lim_{t\to\infty}\norm{\fe}\le\tfrac{\overline\tau_d}{\lambda_{\min}(\mm K)}$, provided that the lumped unknown system $\mv{f}$ can be perfectly cancelled by the control input $\rbu$, neglecting the effect of input saturation.
Hence, the ideal control input $\rbu^*$ can be designed as $-\mv f$.
However, $\rbu^*$ is not available in practice, since \DIFaddbegin \DIFadd{the }\DIFaddend system dynamics encapsulated in $\mv f$ are unknown.
...
\begin{equation}
    ...
    \label{eq:lya1:dot2}
\end{equation}
\DIFdelbegin \DIFdel{Therefore, one can conclude that the result of \eqref{eq:lya1:dot1} can be obtained }\DIFdelend \DIFaddbegin \DIFadd{It is concluded that the filtered error can be stabilized
Therefore, it is concluded that the filtered error $\fe$ can be stabilized }\DIFaddend by adapting $\estwth$ to $\idealwth$, \ie $\hat\NN\to\idealNN$, neglecting the effect of input saturation.
...
\begin{figure}[t]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.99\linewidth]{src/figures/DNN.drawio.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.8\linewidth]{src/figures/DNN.drawio.pdf}
    \DIFaddendFL \caption{
...
The DNN architecture $\NN({\q}_n;\wth) := \NN_k$ can be recursively represented from the \DIFdelbegin \DIFdel{$0\textsuperscript{th}$ }\DIFdelend \DIFaddbegin \DIFadd{$0$th }\DIFaddend input layer to the \DIFdelbegin \DIFdel{$k\textsuperscript{th}$ }\DIFdelend \DIFaddbegin \DIFadd{$k$th }\DIFaddend output layer, as follows:
\begin{equation}
    \NN_i :=
    \begin{cases}
...
\end{equation}
where $\wV_i=[w_{ij}]_{i\in\{1,\cdots,l_i+1\},j\in\{1,\cdots,l_{i+1}\}}\in\R^{(l_i+1)\times l_{i+1}}$ and $\act_i: \R^{l_i}\to\R^{l_i+1}$ represent the weight matrix and the activation function of the \DIFdelbegin \DIFdel{$i\textsuperscript{th}$ }\DIFdelend \DIFaddbegin \DIFadd{$i$th }\DIFaddend layer, respectively.
...
In selecting the nonlinear function $\sigma(\cdot)$, the ReLU function \cite{Maas:2013aa} is one of \DIFdelbegin \DIFdel{the widely used functions }\DIFdelend \DIFaddbegin \DIFadd{most widely used choices }\DIFaddend for large DNNs, since it effectively \DIFdelbegin \DIFdel{avoids }\DIFdelend \DIFaddbegin \DIFadd{mitigates }\DIFaddend the gradient vanishing problem during error backpropagation. 
However, for control applications, where relatively shallow DNNs are typically sufficient \DIFdelbegin \DIFdel{, }\DIFdelend and the gradient vanishing issue is less severe, the sigmoid function or the hyperbolic tangent function is commonly used as the nonlinear function $\sigma(\cdot)$. 
...
For simplicity, each layer's weights are vectorized as $\wth_i:=\myvec(\wV_i)\in\R^{\Xi_i},\forall i\in\{0,\cdots,k\}$, where $\Xi_i:= (l_i+1)l_{i+1}$ is the number of weights in the \DIFdelbegin \DIFdel{$i\textsuperscript{th}$ }\DIFdelend \DIFaddbegin \DIFadd{$i$th }\DIFaddend layer. 
...
In the following sections, let $\idealNN_i$ represent the output of the \DIFdelbegin \DIFdel{$i\textsuperscript{th}$ }\DIFdelend \DIFaddbegin \DIFadd{$i$th }\DIFaddend layer with the ideal weight vector $\idealwth$. 
\DIFdelbegin \DIFdel{Additionally, let us define }\DIFdelend \DIFaddbegin \DIFadd{Define }\DIFaddend $\idealact_i:=\act_i(\idealNN_{i-1})$ and ${\idealact}'_i:= \ddtfrac{\idealact_i}{\idealNN_{i-1}}$. 
Similarly, \DIFaddbegin \DIFadd{let }\DIFaddend $\estNN_i$ denotes the output of the \DIFdelbegin \DIFdel{$i\textsuperscript{th}$ }\DIFdelend \DIFaddbegin \DIFadd{$i$th }\DIFaddend layer with the estimated weight vector $\estwth$, and $\estact_i:=\act_i(\estNN_{i-1})$ and \DIFaddbegin \DIFadd{define }\DIFaddend $\estact_i':= \ddtfrac{\estact_i}{\estNN_{i-1}}$.
...
The recommended method to calculate the exact value of $\pptfrac{\fe}{\rbu}$ is to use the forward sensitivity method \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Sengupta:2014aa} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Sengupta:2014aa,Ryu:2024aa} }\hskip0pt%DIFAUXCMD
}\DIFaddend by simulating the sensitivity equation as follows: 
\begin{equation}
    \ddtt 
    \left(
        \pptfrac{\fe}{\rbu}
...
\begin{remark}
    ...
    Moreover, Assumption \ref{assum:LICQ} is a standard assumption in optimization problems, \DIFdelbegin \DIFdel{which ensures }\DIFdelend \DIFaddbegin \DIFadd{ensuring }\DIFaddend that the gradients of the active constraints are linearly independent.
...
Since $\rbu = \estNN$, a linear mapping $\mm T(\cdot):\estwth_k\to\rbu$\DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{, which }\DIFaddend is independent of $\estwth_k$, can be derived using Proposition \ref{propsit:kron}:
...
For instance, if $k=3$ and $i=1$, $\norm{\pptfrac{c_j}{\estwth_1}}$ is bounded, provided that \DIFdelbegin \DIFdel{$\norm{\estwth_3},\norm{\estwth_2}$ }\DIFdelend \DIFaddbegin \DIFadd{$\norm{\estwth_3}$ and $\norm{\estwth_2}$ }\DIFaddend are bounded.
...
Consequently, the boundedness of $\norm{\pptfrac{c_j}{\estwth_i}},\ \forall j\in\mathcal I \setminus \{\wth_i\}_{i\in\{0,\cdots,k\}}$ \DIFdelbegin \DIFdel{is }\DIFdelend depends on the boundedness of $\estwth_i,\ \forall i\in \{k,\cdots,i+1\}$.
...
\begin{theorem}
    ...
    \DIFdelbegin \DIFdel{This holds under the weight norm constraints \eqref{eq:cstr:weight:ball}.
}\DIFdelend \DIFaddbegin \ctxt{red}{
        This holds under the weight norm constraints \eqref{eq:cstr:weight:ball}.
    }
    \KHCH{
        이렇게 표현하면 마치 이 constraints만 있다고 받아들일 수도 있습니다. 꼭 이 표현을 써야할 이유가 있을까요? 없으면 빼주시고 있으면 constraint input constraints도 추가하면 좋겠습니다.
    }
    \MSRY{
        필요조건으로서 weight norm constraint가 필요하여서 this holds ...로 표했습니다. 만일 교수님이 말씀하신데로 혼동이 될 수 있다면, ..., provided that the weight norm constraints \eqref{eq:cstr:weight:ball} are imposed. 로 바꿀 필요가 있을 것으로 보입니다.
    }
\DIFaddend \end{theorem}

\begin{proof}

%DIF <  The boundednesses of $\estwth$ and $\fe$ are analyzed recursively from the last $k\textsuperscript{th}$ layer to the first layer of $\estNN$. 
...
\end{equation}
It is notable that the ideal DNN \DIFdelbegin \DIFdel{$\idealNN_k={\idealwV_k}^\top\idealact$ }\DIFdelend \DIFaddbegin \DIFadd{$\idealNN_k={\idealwV_k}^\top\idealact_k$ }\DIFaddend is bounded by $\overline{\theta}_k^*\sqrt{l_{k}+1}$.
This is because, the ideal weight $\idealwth_k$ is bounded \DIFdelbegin \DIFdel{according to the universal approximation theorem in Section \ref{sec:sub:NAC} and }\DIFdelend \DIFaddbegin \DIFadd{by }\DIFaddend Assumption \ref{assum:feasible} such that $\norm{\idealwth_k}\le\overline{\theta}_k^*$ for some positive constant $\overline{\theta}_k^*$.
Hence, invoking $\norm{\idealwV_k}_F = \norm{\idealwth_k} \le \overline{\theta}_k^*$ \DIFdelbegin \DIFdel{, }\DIFdelend and $\norm{\idealact_k}\le \sqrt{l_{k}+1}$, we have $\norm{\idealNN}\le \overline{\theta}_k^*\sqrt{l_{k}+1}$.
...
For the weight norm constraint $c_{\theta_k}$, if the corresponding Lagrange multiplier $\lambda_{\theta_k}$ increases sufficiently large \DIFdelbegin \DIFdel{, }\DIFdelend due to the constraint violation, $\estwth_k$ approaches to the origin \DIFdelbegin \DIFdel{, }\DIFdelend as the invariant set $\Theta_{\theta_k}^{1}$ \DIFdelbegin \DIFdel{shrinks to a point}\DIFdelend \DIFaddbegin \DIFadd{toward the origin}\DIFaddend .
The remaining control input constraints can be validated implicitly through the term ${\gamma}_2$ in \eqref{eq:lya2:dot}.
Similarly to $c_{\theta_k}$, when a control input constraint $c_j,\ \forall j\in\mathcal{I}\setminus \{\wth_i\}_{i\in\{0,\cdots,k\}}$\DIFaddbegin \DIFadd{, }\DIFaddend is violated, the corresponding Lagrange multiplier $\lambda_j$ increases sufficiently large, leading to \DIFaddbegin \DIFadd{an }\DIFaddend increase in ${\gamma}_2$.
It makes ${\gamma}_2$ dominates the right-hand side of \eqref{eq:lya2:dot}, which leads $\ddtt V_2$ negative definite.
This drives $\estwth_k$ \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{toward }\DIFaddend the origin until the constraint is satisfied.
...
Therefore, the boundedness of $\fe$ and $\estwth_k$ is guaranteed by the invariant sets $\Theta_{\fe}^{1}$ and $\Theta_{\theta_k}^{1}$, and the satisfactions of $c_j,\ \forall j\in\mathcal{I}$\DIFaddbegin \DIFadd{, }\DIFaddend is ensured.

\subsection*{Case 2: Control input is not saturated.}

Since \DIFdelbegin \DIFdel{, }\DIFdelend the control input constraints are inactive, the saturation function $\mysat(\cdot)$ in \eqref{eq:lya1:dot2} and the constraint functions $c_j,\ \forall j\in\mathcal I \setminus \{\wth_i\}_{i\in\{0,\cdots,k\}}$\DIFaddbegin \DIFadd{, }\DIFaddend in \eqref{eq:lya2:dot} can be \DIFdelbegin \DIFdel{removed}\DIFdelend \DIFaddbegin \DIFadd{neglected}\DIFaddend .
...
\DIFdelend \DIFaddbegin \begin{aligned}
    ...
    \label{eq:lya3:dot}
\end{equation}
\DIFdelbegin \DIFdel{Following }\DIFdelend \DIFaddbegin \DIFadd{Based on }\DIFaddend the same reasoning as in Case 1, the boundedness of $\fe$ is ensured by the invariant set:
\begin{equation}
    \Theta_{\fe}^{2}
    =
    \left\{ 
...
\begin{equation}
    ...
    \label{eq:estwth:dyn}
\end{equation}
By Lemma \ref{lem:cstr:grad:bound}\DIFaddbegin \DIFadd{, }\DIFaddend $\estwth_i$ is bounded, provided that $\estwth_i,\ \forall i\in\{k,\cdots,i+1\}$, are bounded\DIFdelbegin \DIFdel{, since }\DIFdelend \DIFaddbegin \DIFadd{. 
This holds because }\DIFaddend the system matrix $-\lambda_{\theta_i} \mm I_{\Xi_i}$ is stable\DIFaddbegin \DIFadd{, }\DIFaddend and the residual terms\DIFdelbegin \DIFdel{are bounded, }%DIFDELCMD < \ie %%%
\DIFdel{$\pptfrac{\estNN}{\estwth_i}, \fe, \pptfrac{c_j}{\estwth_i}$ are bounded}\DIFdelend \DIFaddbegin \DIFadd{—such as $\pptfrac{\estNN}{\estwth_i}, \fe$, }\DIFaddend and \DIFaddbegin \DIFadd{$\pptfrac{c_j}{\estwth_i}$ —are bounded.
Moreover, the Lagrange multiplier }\DIFaddend $\lambda_j$ does not diverge since the control input constraints \DIFdelbegin \DIFdel{will be satisfied before the divergence occurs}\DIFdelend \DIFaddbegin \DIFadd{are satisfied before divergence can occur}\DIFaddend .
Therefore, starting from the \DIFdelbegin \DIFdel{$(k-1)\textsuperscript{th}$ }\DIFdelend \DIFaddbegin \DIFadd{$(k-1)$th }\DIFaddend layer, the boundedness of $\estwth_i$ can be established recursively down to the input layer ($i=0$).
\DIFdelbegin \DIFdel{And }\DIFdelend \DIFaddbegin \DIFadd{Moreover }\DIFaddend the satisfaction of the weight norm constraints \DIFdelbegin \DIFdel{of inner weights }\DIFdelend \DIFaddbegin \DIFadd{for the inner layers }\DIFaddend can be verified\DIFaddbegin \DIFadd{, }\DIFaddend as the Lagrange multipliers $\lambda_{\theta_i}$, $\forall i\in\{k-1,\cdots,0\}$, increase sufficiently large to stabilize the $\estwth_i$ dynamics \DIFaddbegin \DIFadd{in }\DIFaddend \eqref{eq:estwth:dyn}, leading to \DIFdelbegin \DIFdel{the convergence of $\widehat{\mv{\theta}}_i$ }\DIFdelend \DIFaddbegin \DIFadd{its convergence }\DIFaddend toward the origin.

\hfill

In conclusion, the filtered tracking error $\fe$ and the weight estimate $\estwth$ are bounded, and all imposed constraints $c_j,\ \forall j\in\mathcal I$\DIFaddbegin \DIFadd{, }\DIFaddend are satisfied.
...
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.8\linewidth]
...
%DIFDELCMD <         \includegraphics[width=0.55\linewidth]{src/figures/RobotModel.drawio.pdf}
%DIFDELCMD <         \label{fig:robot:model}
%DIFDELCMD <     }
%DIFDELCMD <     \hfill
%DIFDELCMD <     \subfloat[Control input saturation function.]{
%DIFDELCMD <         \includegraphics[width=0.6\linewidth]{src/figures/saturation.drawio.pdf}
%DIFDELCMD <         \label{fig:robot:sat}
%DIFDELCMD <     }
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[Two-link robotic manipulator.]{
    ...
    \subfloat[Control input saturation function.]{
        \includegraphics[width=0.5\linewidth]{src/figures/saturation.drawio.pdf}
        \label{fig:robot:sat}
    }
    \DIFaddendFL \caption{Two-link robotic manipulator and control saturation function.}
    \label{fig:robot}
\end{figure}
...
\end{equation}
where $\mathcal{P}^5(t;\mv{x}_1,\mv{x}_2,t_f)$ denotes a quintic (\DIFdelbegin \DIFdel{5$\textsuperscript{th}$-order}\DIFdelend \DIFaddbegin \DIFadd{$5$th-order}\DIFaddend ) polynomial trajectory that moves the system from $\mv{x}_1\in\R^2$ to $\mv{x}_2\in\R^2$ in $t_f$ seconds, with zero initial and final velocities.
...
\begin{table}[t]
    ...
    \centering
    \DIFdelbeginFL %DIFDELCMD < \begin{tabular}{c l l}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \begin{tabular}{m{.25cm} m{4cm} l}
    %DIF >  \begin{tabular}{c l l}
    \DIFaddendFL \hline
        &\bf{Description}&\bf{Input Constraint Handling}\\
    \hline
    \hline
        \DIFdelbeginFL %DIFDELCMD < \multirow{2}{*}{(C$_1$)}%%%
\DIFdelendFL \DIFaddbeginFL \multirow{3}{*}{(C$_1$)}\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{large}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{CONAC with }\DIFaddendFL & \eqref{eq:cstr:input:bound} \DIFdelbeginFL \DIFdelFL{$c_{\overline\tau_2}, c_{\underline\tau_2}$ with $\overline{\tau}_2=-\underline{\tau}_2=3.5$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$c_{\overline\tau_2}$ and $c_{\underline\tau_2}$ }\DIFaddendFL \\
        & \DIFaddbeginFL \DIFaddFL{large }\DIFaddendFL update rate $\beta_j$\DIFaddbeginFL \DIFaddFL{. }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{with $\overline{\tau}_2=-\underline{\tau}_2=3.5$, and }\\
        & \DIFaddFL{($\beta_{\overline{\tau}_2}=\beta_{\underline{\tau}_2}=100$ and $\beta_{\rbu}=10$) }& \DIFaddendFL \eqref{eq:cstr:input:norm} $c_{\rbu}$ with $\overline{\tau}=11$\DIFaddbeginFL \DIFaddFL{. }\DIFaddendFL \\
    \hline
        \DIFdelbeginFL %DIFDELCMD < \multirow{2}{*}{(C$_2$)}%%%
\DIFdelendFL \DIFaddbeginFL \multirow{3}{*}{(C$_2$)}\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{small}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{CONAC with }\DIFaddendFL & \eqref{eq:cstr:input:bound} \DIFdelbeginFL \DIFdelFL{$c_{\overline\tau_2}, c_{\underline\tau_2}$ with $\overline{\tau}_2=-\underline{\tau}_2=3.5$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$c_{\overline\tau_2}$ and $c_{\underline\tau_2}$ }\DIFaddendFL \\
        & \DIFaddbeginFL \DIFaddFL{small }\DIFaddendFL update rate $\beta_j$\DIFaddbeginFL \DIFaddFL{. }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{with $\overline{\tau}_2=-\underline{\tau}_2=3.5$, and }\\
        & \DIFaddFL{($\beta_{\overline{\tau}_2}=\beta_{\underline{\tau}_2}=1$ and $\beta_{\rbu}=0.1$) }& \DIFaddendFL \eqref{eq:cstr:input:norm} $c_{\rbu}$ with $\overline{\tau}=11$\DIFaddbeginFL \DIFaddFL{. }\DIFaddendFL \\
    \hline
        \DIFdelbeginFL %DIFDELCMD < \multirow{2}{*}{(C$_3$)}%%%
\DIFdelendFL \DIFaddbeginFL \multirow{3}{*}{(C$_3$)}\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{no input }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{CONAC without}\DIFaddendFL & \DIFdelbeginFL %DIFDELCMD < \multirow{2}{*}{$[\beta_j]_{j\in\{\overline{\tau}_2,\underline{\tau}_2,\rbu\}}=\mv{0}$} %%%
\DIFdelendFL \DIFaddbeginFL \multirow{3}{*}{ \shortstack[l]{No input constraints\\are imposed.} } \DIFaddendFL \\
        \DIFaddbeginFL & \DIFaddFL{input constraints. }\DIFaddendFL &  \DIFdelbeginFL \DIFdelFL{constraint }\DIFdelendFL \DIFaddbeginFL \\
        \DIFaddendFL & \DIFaddbeginFL \DIFaddFL{($\beta_{\overline{\tau}_2}=\beta_{\underline{\tau}_2}=\beta_{\rbu}=0$) }& \DIFaddendFL \\
    \hline
        \multirow{2}{*}{(C$_4$)}& \DIFdelbeginFL %DIFDELCMD < \multirow{2}{*}{auxiliary system} %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{NAC with }\DIFaddendFL & $\overline{\tau}_1=-\underline{\tau}_1=10.428$ \\
        & \DIFaddbeginFL \DIFaddFL{auxiliary system. }\DIFaddendFL & $\overline{\tau_2}=-\underline{\tau_2}=3.5$\DIFaddbeginFL \DIFaddFL{. }\DIFaddendFL \\    
    \hline
    \end{tabular}
...
% ------------------------------------
\subsubsection*{Controller 1 \DIFdelbegin \DIFdel{: }\DIFdelend \normalfont{(C$_1$)}\DIFdelbegin \textit{\DIFdel{CONAC with large update rate, $\beta_j,\forall j\in\mathcal{I}$, of Lagrange Multipliers}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{: }\textit{\DIFadd{CONAC with a large update rate $\beta_j,\forall j\in\mathcal{I}$, for the Lagrange Multipliers}}\DIFaddend }
\DIFaddbegin \MSRY{
    끝이 :은 subsubsection으로 자동으로 붙었습니다. 앞의 :을 -와 같은 것으로 바꿀까요?
}
\DIFaddend 
...
For the control input constraints, \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{large }\DIFaddend update rates were \DIFdelbegin \DIFdel{assigned large values}\DIFdelend \DIFaddbegin \DIFadd{used to examine their effect}\DIFaddend : $\beta_{\overline{\tau}2} = \beta{\underline{\tau}_2} = 10^2$ and $\beta_{\rbu} = 10$, in order to investigate the effect of large update rates.

% ------------------------------------
% Controllers: CONAC with small beta
% ------------------------------------
\subsubsection*{Controller 2 \DIFdelbegin \DIFdel{: }\DIFdelend \normalfont{(C$_2$)}\DIFdelbegin \textit{\DIFdel{CONAC with small update rate, $\beta_j,\forall j\in\mathcal{I}$, of Lagrange Multipliers}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{: }\textit{\DIFadd{CONAC with a small update rate $\beta_j,\forall j\in\mathcal{I}$, for the Lagrange Multipliers}}\DIFaddend }

\DIFdelbegin \DIFdel{As the second controller, the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend proposed CONAC was implemented with the same input bound constraints as \DIFaddbegin \DIFadd{in }\DIFaddend (C$_1$), but with \DIFaddbegin \DIFadd{update rates for the $[\lambda_j]_{j\in\{\overline{\tau}_2,\underline{\tau}_2,\rbu\}}$reduced by a factor of }\DIFaddend $100$\DIFdelbegin \DIFdel{times smaller update rate values}\DIFdelend : $\beta_{\overline{\tau}_2}=\beta_{\underline{\tau}_2}=1$ and $\beta_{\rbu}=0.1$.
All other properties and parameters were kept identical to those of (C$_1$).

% ------------------------------------
% Controllers: CONAC without ctrl cstr
% ------------------------------------
\subsubsection*{Controller 3 \DIFdelbegin \DIFdel{: }\DIFdelend \normalfont{(C$_3$)}\DIFaddbegin \DIFadd{: }\DIFaddend \textit{CONAC without control input constraints}}
...
% ------------------------------------
\subsubsection*{Controller 4 \DIFdelbegin \DIFdel{: }\DIFdelend \normalfont{(C$_4$)}\DIFdelbegin \textit{\DIFdel{CONAC with auxiliary system instead of control input constraints}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{: }\textit{\DIFadd{NAC with auxiliary system}} \DIFadd{(conventional method)}\DIFaddend }

As a baseline for comparison, the \DIFdelbegin \DIFdel{existing auxiliary system approach }\DIFdelend \DIFaddbegin \DIFadd{auxiliary system-based approach from }\DIFaddend \cite{Esfandiari:2014aa, Karason:1994aa, Esfandiari:2015aa} was implemented\DIFdelbegin \DIFdel{in place of the proposed CONAC for handling control input constraints.
}\DIFdelend \DIFaddbegin \DIFadd{.
%DIF >  in place of the proposed CONAC for handling control input constraints.
}\DIFaddend The auxiliary system is defined by $\ddtt{\mv\zeta} = \mm{A}_\zeta \mv{\zeta} + \mm{B}_\zeta \Delta\rbu$, with initial condition $\mv{\zeta}\vert_{t=0} = \mv{0}_{2\times1}$, where $\mv{\zeta}\in\R^2$ represents the auxiliary state variables, and $\mm{A}_\zeta\in\R_{<0}^{2\times 2}$ and $\mm{B}_\zeta\in\R^{2\times 2}$ are user-designed matrices.
...
As a result, the weights were adapted to minimize both the filtered tracking error and the auxiliary state, thereby \DIFdelbegin \DIFdel{resolving the control input saturation for }\DIFdelend \DIFaddbegin \DIFadd{mitigating saturation in }\DIFaddend each control input.

The \DIFdelbegin \DIFdel{matrices for the auxiliary system }\DIFdelend \DIFaddbegin \DIFadd{auxiliary system matrices }\DIFaddend were chosen as \DIFdelbegin \DIFdel{$\mm{A}_\zeta=\mydiag\left((-10,-10)^\top\right)$ and $\mm{B}_\zeta=\mydiag\left((10^3,10^3)^\top\right)$, 
reflecting a trade-off between sensitivity to input overflow and }\DIFdelend \DIFaddbegin \DIFadd{$\mm{A}_\zeta=\mydiag(-10,-10)$ and $\mm{B}_\zeta=\mydiag(10^3,10^3)$, 
to balance responsiveness to input overflow with the }\DIFaddend convergence speed of the auxiliary \DIFdelbegin \DIFdel{system }\DIFdelend dynamics.
Since the auxiliary system only accounts for \DIFdelbegin \DIFdel{individual }\DIFdelend \DIFaddbegin \DIFadd{element-wise }\DIFaddend input saturation—corresponding to the input bound constraint \eqref{eq:cstr:input:bound}—the upper \DIFdelbegin \DIFdel{limit for }\DIFdelend \DIFaddbegin \DIFadd{bound of }\DIFaddend the first actuator torque \DIFdelbegin \DIFdel{, $\overline{\tau}_1$ which is equal to $-\underline{\tau}_1$, was set }\DIFdelend \DIFaddbegin \DIFadd{$\overline{\tau}_1=-\underline{\tau}_1$ was adjusted }\DIFaddend to $10.428$ to \DIFdelbegin \DIFdel{approximately match the allowable control input norm $\overline{\tau}$, given $\overline{\tau}_2=-\underline{\tau}_2 = 3.5$ and $\overline{\tau} = 11$, satisfying }\DIFdelend \DIFaddbegin \DIFadd{approximate the input norm constraint $\overline{\tau} = 11$, given }\DIFaddend $\overline{\tau}_1^2 + \overline{\tau}_2^2 = \overline{\tau}^2$.

\DIFaddbegin \DIFadd{All other properties and parameters were kept the same as in (C$_1$), (C$_2$) and (C$_3$) expect that the weight boundedness was handled using the projection operator used in \mbox{%DIFAUXCMD
...
All DNNs shared the same architecture, consisting of two hidden layers with four nodes each, \ie $k=2$\DIFaddbegin \DIFadd{, }\DIFaddend and $l_0=6, l_1=4, l_2=4$, and $l_3=2$ in \eqref{eq:DNN:architecture}.
The adaptation gain was set to $\alpha = 0.2$, and the filtering matrix in \eqref{eq:err:filtered} was chosen as \DIFdelbegin \DIFdel{$\mm{\Lambda}=\mydiag\left((5,15)^\top\right)$}\DIFdelend \DIFaddbegin \DIFadd{$\mm{\Lambda}=\mydiag(5,15)$}\DIFaddend .
...
\begin{figure*}[t]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \subfloat[Tracking result of $1$\textsuperscript{st} joint angle $q_1$.]{
...
\DIFdelendFL \DIFaddbeginFL \subfloat[Tracking performance of $1$st joint angle $q_1$.]{
    ...
    \DIFaddendFL \hfill
    \DIFdelbeginFL %DIFDELCMD < \subfloat[Tracking result of $2$\textsuperscript{nd} joint angle $q_2$.]{
%DIFDELCMD <       \includegraphics[width=\figSizeTwoCol\linewidth]
%DIFDELCMD <       {
...
%DIFDELCMD <     %%%
\DIFdelendFL %DIF >  \vfill
    \DIFaddbeginFL \subfloat[Tracking performance of $2$nd joint angle $q_2$.]{
        \includegraphics[width=\figSizeTwoCol\linewidth]
        {
    ...
    \DIFaddendFL \vfill
    \DIFdelbeginFL %DIFDELCMD < \subfloat[Control input $1$\textsuperscript{st} joint torque $\tau_1$.]{
%DIFDELCMD <         \includegraphics[width=\figSizeTwoCol\linewidth]
%DIFDELCMD <         {
...
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[Control input of the $1$st joint $\tau_1$.]{
    \includegraphics[width=\figSizeTwoCol\linewidth]
        {
    ...
    \DIFaddendFL \hfill
    \DIFdelbeginFL %DIFDELCMD < \subfloat[Control input $2$\textsuperscript{nd} joint torque $\tau_2$.]{
%DIFDELCMD <         \includegraphics[width=\figSizeTwoCol\linewidth]
%DIFDELCMD <         {
...
%DIFDELCMD <         %%%
\DIFdelendFL %DIF >  \vfill
        \DIFaddbeginFL \subfloat[Control input of the 2nd joint $\tau_2$.]{
        \includegraphics[width=\figSizeTwoCol\linewidth]
        {
    ...
    \DIFaddendFL \vfill
    \DIFdelbeginFL %DIFDELCMD < \subfloat[Norm of control input (joint torque)$\rbu$.]{
%DIFDELCMD <         \includegraphics[width=\figSizeTwoCol\linewidth]
%DIFDELCMD <         {
...
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[Norm of the joint torque input $\rbu$.]{
        \includegraphics[width=\figSizeTwoCol\linewidth]
        {
...
Furthermore, the stable learning \DIFdelbegin \DIFdel{achieved }\DIFdelend \DIFaddbegin \DIFadd{observed }\DIFaddend under random weight initialization and \DIFdelbegin \DIFdel{no }\DIFdelend \DIFaddbegin \DIFadd{without }\DIFaddend prior knowledge of the system, demonstrated the effectiveness of the proposed CONAC in enabling online learning capability.
...
The tracking performance of CONAC with control input constraints—\ie (C$_1$) and (C$_2$)—outperformed the auxiliary system-based \DIFdelbegin \DIFdel{CONAC }\DIFdelend \DIFaddbegin \DIFadd{NAC }\DIFaddend (C$_4$) in the second episode, as shown in Table~\ref{tab:sim:L2}.
This improvement is \DIFdelbegin \DIFdel{due to the fact that }\DIFdelend \DIFaddbegin \DIFadd{attributed to the ability of }\DIFaddend (C$_1$) and (C$_2$) \DIFdelbegin \DIFdel{were able }\DIFdelend to fully utilize the actuator's \DIFdelbegin \DIFdel{capabilities}\DIFdelend \DIFaddbegin \DIFadd{capacity}\DIFaddend , whereas (C$_4$) was \DIFdelbegin \DIFdel{constrained by the auxiliary system's design, which limited }\DIFdelend \DIFaddbegin \DIFadd{limited by the design of the auxiliary system, which constrained }\DIFaddend the first joint actuator.
...
In addition, the \DIFdelbegin \DIFdel{magnitude }\DIFdelend \DIFaddbegin \DIFadd{severity }\DIFaddend of saturation violations \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend (C$_3$) increased during \DIFdelbegin \DIFdel{each interval where }\DIFdelend \DIFaddbegin \DIFadd{periods when the }\DIFaddend input constraints were active. 
\DIFdelbegin \DIFdel{Such large violations may lead to actuator damage or failure, posing }\DIFdelend \DIFaddbegin \DIFadd{These large violations pose }\DIFaddend a significant risk to the physical system\DIFdelbegin \DIFdel{.
Moreover}\DIFdelend \DIFaddbegin \DIFadd{, potentially causing actuator damage or failure. 
Furthermore}\DIFaddend , once the control saturation was \DIFdelbegin \DIFdel{released}\DIFdelend \DIFaddbegin \DIFadd{lifted}\DIFaddend —typically due to \DIFdelbegin \DIFdel{a change }\DIFdelend \DIFaddbegin \DIFadd{changes }\DIFaddend in the desired trajectory—\DIFdelbegin \DIFdel{these large violations }\DIFdelend \DIFaddbegin \DIFadd{this }\DIFaddend resulted in oscillatory behavior, as CONAC's weights had been adapted to \DIFdelbegin \DIFdel{produce }\DIFdelend \DIFaddbegin \DIFadd{generate }\DIFaddend excessively high control inputs,\DIFdelbegin \DIFdel{as shown in Fig.~\ref{fig:ctrl:result:tau:1}–\ref{fig:ctrl:result:tau:norm}.
}\DIFdelend \DIFaddbegin \ctxt{red}{ as illustrated in Fig.~\ref{fig:ctrl:result:tau:1}–\ref{fig:ctrl:result:tau:norm}.}
\DIFaddend 
...
\begin{figure}[t]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \subfloat[Control input $\tau$ locus in time interval from $29.4$ s to $31.9$ s.]{
...
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[Control input $\rbu$ locus during the time interval from $29.4$ s to $31.9$ s.]{
    % \subfloat[Auxiliary states $\mv{\zeta}$ of CONAC-AUX.]{
        ...
        \DIFaddendFL \vfill
    \DIFdelbeginFL %DIFDELCMD < \subfloat[Lagrange multipliers $\lambda_j,\forall j\in\mathcal{I}$ in time interval from $29.4$ s to $31.9$ s.]{
%DIFDELCMD <         \includegraphics[width=\figSizeOneCol\linewidth]
%DIFDELCMD <         {
...
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[Lagrange multipliers $\lambda_j$ during the time interval from $29.4$ s to $31.9$ s.]{
        \includegraphics[width=\figSizeOneCol\linewidth]
        {
        ...
        \DIFaddendFL \vfill
    \DIFdelbeginFL %DIFDELCMD < \subfloat[Auxiliary states in time interval from $29.4$ s to $31.9$ s.]{
%DIFDELCMD <         \includegraphics[width=\figSizeOneCol\linewidth]
%DIFDELCMD <         {
...
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[Auxiliary states $\mv{\zeta}$ during the time interval from $29.4$ s to $31.9$ s.]{
        \includegraphics[width=\figSizeOneCol\linewidth]
        {
    ...
    \DIFaddendFL \caption{
    Constraint handling \DIFdelbeginFL \DIFdelFL{process }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{behavior }\DIFaddendFL of (C$_1$) \protect\colorLine{blue}{solid}, (C$_2$) \protect\colorLine{cyan}{solid}, (C$_3$) \protect\colorLine{gray}{solid}, and (C$_4$) \protect\colorLine{magenta}{solid} \DIFdelbeginFL \DIFdelFL{in }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{during the }\DIFaddendFL time interval from $29.4$ s to $31.9$ s.
    }
...
\begin{figure}[!t]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.7\linewidth]{
%DIFDELCMD < 		src/figures/lambda_effect.drawio.pdf
%DIFDELCMD < 		}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.6\linewidth]{
...
As shown in Fig.~\ref{fig:ctrl:result:tau:1}–\ref{fig:ctrl:result:tau:norm}, all controllers except (C$_3$)\DIFaddbegin \DIFadd{, }\DIFaddend which did not consider the control input saturation, satisfied the control input saturation illustrated in Fig.~\ref{fig:robot:sat}.
\DIFdelbegin \DIFdel{For }\DIFdelend \DIFaddbegin \DIFadd{In }\DIFaddend (C$_1$) and (C$_2$), \DIFdelbegin \DIFdel{the }\DIFdelend control input saturation was \DIFdelbegin \DIFdel{handled using }\DIFdelend \DIFaddbegin \DIFadd{addressed through }\DIFaddend corresponding input constraints \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{within }\DIFaddend the constrained optimization framework\DIFdelbegin \DIFdel{and, for }\DIFdelend \DIFaddbegin \DIFadd{, whereas }\DIFaddend (C$_4$) \DIFdelbegin \DIFdel{, the auxiliary systemwas utilized}\DIFdelend \DIFaddbegin \DIFadd{employed an auxiliary system}\DIFaddend , as summarized in Table~\ref{table:controllers}.
...
In Fig.~\ref{fig:ctrl:result:scope}, the control input locus, the Lagrange multipliers, and the auxiliary states are illustrated \DIFdelbegin \DIFdel{during }\DIFdelend \DIFaddbegin \DIFadd{over }\DIFaddend this time interval.
The properties of the controllers \DIFdelbegin \DIFdel{can be observed }\DIFdelend \DIFaddbegin \DIFadd{are illustrated }\DIFaddend in Fig.~\ref{fig:ctrl:result:scope:control}, where the control input \DIFdelbegin \DIFdel{locus }\DIFdelend \DIFaddbegin \DIFadd{loci }\DIFaddend of (C$_1$) and (C$_2$) fully \DIFdelbegin \DIFdel{explores the feasible domain of control input illustrated }\DIFdelend \DIFaddbegin \DIFadd{explore the feasible input domain shown }\DIFaddend in Fig.~\ref{fig:robot:sat}, \DIFdelbegin \DIFdel{while }\DIFdelend \DIFaddbegin \DIFadd{whereas }\DIFaddend (C$_3$) exceeds the input saturation limits, and (C$_4$) is \DIFdelbegin \DIFdel{limited }\DIFdelend \DIFaddbegin \DIFadd{restricted }\DIFaddend in its first actuator input \DIFdelbegin \DIFdel{by }\DIFdelend \DIFaddbegin \DIFadd{limit due to }\DIFaddend the auxiliary system design.

% beta
The \DIFdelbegin \DIFdel{detail of the }\DIFdelend input constraint handling \DIFdelbegin \DIFdel{during the time interval from }\DIFdelend \DIFaddbegin \DIFadd{between }\DIFaddend $29.4$ s to $31.9$ s \DIFdelbegin \DIFdel{, is now investigated. 
In the cases of }\DIFdelend \DIFaddbegin \DIFadd{is examined in detail. 
For }\DIFaddend (C$_1$) and (C$_2$), \DIFdelbegin \DIFdel{as the }\DIFdelend \DIFaddbegin \DIFadd{activation of }\DIFaddend input constraints $c_j$ \DIFdelbegin \DIFdel{were activated, }\DIFdelend \DIFaddbegin \DIFadd{dynamically generated }\DIFaddend the corresponding Lagrange multipliers $\lambda_j$\DIFdelbegin \DIFdel{were generated dynamically}\DIFdelend , as shown in Fig.~\ref{fig:ctrl:result:scope:beta}. 
These \DIFdelbegin \DIFdel{Lagrange multipliers directed }\DIFdelend \DIFaddbegin \DIFadd{multipliers guided }\DIFaddend the adaptation of the weights $\estwth_i,\ \forall i\in\{0,1,2\}$, to reduce \DIFdelbegin \DIFdel{violations of the constraints $c_j$, according to }\DIFdelend \DIFaddbegin \DIFadd{constraint violations in accordance with }\DIFaddend \eqref{eq:adap:th}.

% comparison; big, small beta
\DIFdelbegin \DIFdel{For more details, the growth effect of some convex input constraint's Lagrange multiplier $\lambda_j$ on the }\DIFdelend \DIFaddbegin \DIFadd{To further illustrate this effect, Fig.~\ref{fig:lambda_effect} compares the }\DIFaddend adaptation direction of $\estwth_2$ \DIFdelbegin \DIFdel{is illustrated in Fig.~\ref{fig:lambda_effect}, where }\DIFdelend \DIFaddbegin \DIFadd{under }\DIFaddend two different magnitudes of $\lambda_j$\DIFdelbegin \DIFdel{are considered. 
With this configuration, the difference between (C$_1$) and (C$_2$) can be investigated, as well, }%DIFDELCMD < \ie %%%
\DIFdel{large and small }\DIFdelend \DIFaddbegin \DIFadd{. 
This highlights the influence of the }\DIFaddend update rate $\beta_j$\DIFdelbegin \DIFdel{, since, for instance, (}\DIFdelend \DIFaddbegin \DIFadd{: a larger $\beta_j$ (as in (}\DIFaddend C$_1$)\DIFdelbegin \DIFdel{with large update rate $\beta_j$ leads to a larger Lagrange multiplier }\DIFdelend \DIFaddbegin \DIFadd{) yields a larger }\DIFaddend $\lambda_j$\DIFdelbegin \DIFdel{than (C$_2$) with small update rate, see \eqref{eq:adap:lbd}.
...
%DIFDELCMD < %%%
\DIFdel{As illustrated in Fig.~\ref{fig:lambda_effect}, with large Lagrange multiplier $\lambda_j$, the adaptation direction }\DIFdelend \DIFaddbegin \DIFadd{, resulting in a more aggressive adjustment }\DIFaddend of $\estwth_2$ \DIFdelbegin \DIFdel{is significantly influenced by }\DIFdelend \DIFaddbegin \DIFadd{via }\DIFaddend the term $-\lambda_j\pptfrac{c_j}{\estwth_2}$ in \eqref{eq:adap:th}, \DIFdelbegin \DIFdel{which directs the weights toward satisfactory point of the constraint .
Hence, this results in a more aggressive adjustment of the weights leading to faster constraint }\DIFdelend \DIFaddbegin \DIFadd{thereby accelerating constraint }\DIFaddend satisfaction—\DIFdelbegin \DIFdel{but }\DIFdelend \DIFaddbegin \DIFadd{though }\DIFaddend at the cost of oscillatory \DIFdelbegin \DIFdel{behavior in the learning process. 
In }\DIFdelend \DIFaddbegin \DIFadd{learning behavior. 
...
\DIFadd{These behaviors are clearly observed in }\DIFaddend Fig.~\ref{fig:ctrl:result:scope:control} and Fig.~\ref{fig:ctrl:result:scope:beta}, \DIFdelbegin \DIFdel{the earlier reasoning can be observed, }\DIFdelend where (C$_1$) \DIFdelbegin \DIFdel{with large update rate $\beta_j$ satisfied }\DIFdelend \DIFaddbegin \DIFadd{satisfies }\DIFaddend the input constraint \DIFdelbegin \DIFdel{$c_{\overline{\tau}_1}$ faster }\DIFdelend \DIFaddbegin \DIFadd{$c_{\rbu}$ more quickly }\DIFaddend than (C$_2$)\DIFdelbegin \DIFdel{with repeatedly growing and resetting }\DIFdelend \DIFaddbegin \DIFadd{, which shows repeated growth and reset of }\DIFaddend $\lambda_{\rbu}$. 
\DIFdelbegin \DIFdel{In contrast, with small $\lambda_j$, the adaptation direction of }\DIFdelend \DIFaddbegin \DIFadd{Under Assumptions \ref{assum:convex} and \ref{assum:LICQ}, the convexity of $c_j$ in the }\DIFaddend $\estwth_2$\DIFdelbegin \DIFdel{is less influenced, leading to a more conservative adjustment of the weights adaptation.
Consequently, the constraint satisfaction is achieved more smoothly, but potentially at the cost of slower satisfaction (convergence) , as shown in Fig.
~\ref{fig:ctrl:result:scope:control} and Fig.~\ref{fig:ctrl:result:scope:beta}.
}\DIFdelend \DIFaddbegin \DIFadd{-space (see Lemma~\ref{lem:convex:angle}) further supports this adaptive behavior.
}\DIFaddend 
...
This is because the weights in (C$_1$), (C$_2$), and (C$_4$) were implicitly suppressed by the control input constraints, \DIFdelbegin \DIFdel{whereas (C$_3$)did not incorporate such constraints}\DIFdelend \DIFaddbegin \DIFadd{which were not applied in (C3)}\DIFaddend .
% , \ie by limiting the control input, the weight norms were also limited.
% \MSRY{필요하다면 왜 제어입력 제약하는게 가중치 제약도 되는지 추가 가능}
Furthermore, the constraints on the inner and hidden layer weights were never violated; that is, \DIFaddbegin \DIFadd{both }\DIFaddend $\norm{\estwth_0}$ and $\norm{\estwth_1}$ consistently remained below their limits.
This is attributed to the fact that the backpropagated error \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{being }\DIFaddend scaled by the \DIFdelbegin \DIFdel{gradient of the activation function}\DIFdelend \DIFaddbegin \DIFadd{activation function's gradient}\DIFaddend , which is bounded between $-1$ and $1$ \DIFdelbegin \DIFdel{; see \eqref{eq:NN:grad}}\DIFdelend \DIFaddbegin \DIFadd{(see \eqref{eq:NN:grad})}\DIFaddend .
...
$\overline\theta_2$ at \DIFdelbegin \DIFdel{approximately }\DIFdelend \DIFaddbegin \DIFadd{around }\DIFaddend $30.4$ s, \DIFdelbegin \DIFdel{as shown in }\DIFdelend \DIFaddbegin \DIFadd{(see }\DIFaddend Fig.~\ref{fig:ctrl:result:weight}\DIFdelbegin \DIFdel{, prompting an adjustment in }\DIFdelend \DIFaddbegin \DIFadd{), thereby adjusting }\DIFaddend the adaptation direction of $\estwth_2$ to mitigate the violation of the constraint $c_{\theta_2}$, as shown in Fig.~\ref{fig:ctrl:result:scope:beta}.
...
This paper proposed a constrained optimization-based neuro-adaptive controller (CONAC) for unknown Euler-Lagrange systems, addressing both weight norm and input constraints \DIFdelbegin \DIFdel{through }\DIFdelend \DIFaddbegin \DIFadd{within }\DIFaddend a rigorous optimization framework. 
The stability of \DIFdelbegin \DIFdel{the proposed }\DIFdelend CONAC was analyzed \DIFdelbegin \DIFdel{via }\DIFdelend \DIFaddbegin \DIFadd{using }\DIFaddend Lyapunov theory, demonstrating bounded tracking and weight estimation errors under real-time adaptation.

\DIFdelbegin \DIFdel{The proposed }\DIFdelend CONAC effectively incorporated both convex input constraints and weight norm constraints, ensuring that actuator limitations and neural network weights remained within predefined bounds. 
By embedding these constraints into the optimization process, CONAC ensured \DIFdelbegin \DIFdel{convergence of the weights }\DIFdelend \DIFaddbegin \DIFadd{weight convergence }\DIFaddend in accordance with the Karush-Kuhn-Tucker (KKT) conditions, thereby \DIFdelbegin \DIFdel{achieving }\DIFdelend \DIFaddbegin \DIFadd{ensuring }\DIFaddend both stability and optimality.

\DIFdelbegin \DIFdel{A real-time }\DIFdelend \DIFaddbegin \DIFadd{Real-time }\DIFaddend implementation validated the superior performance of CONAC compared to \DIFdelbegin \DIFdel{conventional methods. 
The proposed }\DIFdelend \DIFaddbegin \DIFadd{a conventional method. 
}\DIFaddend CONAC successfully handled complex input constraints while rigorously managing weight norms, resulting in improved tracking accuracy and stable \DIFdelbegin \DIFdel{performance }\DIFdelend \DIFaddbegin \DIFadd{behavior }\DIFaddend without significant oscillations.
