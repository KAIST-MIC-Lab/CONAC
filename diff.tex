...
%DIF DEL tmp2.tex   Mon Jul 28 00:53:39 2025
%DIF ADD tmp1.tex   Mon Jul 28 00:53:39 2025
...
Various types of NNs have been utilized in NAC, including simpler architectures like single-hidden layer (SHL) NNs \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Lewis:1996aa,Ge:2010aa, Yesildirek:1995aa} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Lewis:1996aa,Ge:2010aa, Yesildirek:1995aa,Ryu:2024aa,Ryu:2025aa} }\hskip0pt%DIFAUXCMD
}\DIFaddend and radial basis function (RBF) NNs \cite{Liu:2013ab,Ge:2002aa}, as well as more complex models like deep NNs (DNNs) \cite{Patil:2022aa} and their variations. 
...
On the other \DIFdelbegin \DIFdel{hands}\DIFdelend \DIFaddbegin \DIFadd{hand}\DIFaddend , in \cite{Esfandiari:2014aa,Karason:1994aa,Esfandiari:2015aa}, the auxiliary systems were incorporated into the adaptation law by adding the auxiliary states to the feedback error for learning.
...
By formulating the NAC problem as \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend unified optimization problem with constraints, it is possible to adapt the NN weights while minimizing an objective function, \eg tracking error, subject to both weight and input constraints, simultaneously.
...
\ctxt{red}
{
    \DIFaddend However, to the best of the authors' knowledge, no prior work has applied constrained optimization to NAC systems with real-time weight adaptation. 
    \DIFaddbegin }
\ctxt{blue}
{
    \DIFadd{However, there are few works that have applied constrained optimization to NAC systems with real-time weight adaptation \mbox{%DIFAUXCMD
\cite{Ryu:2024aa,Ryu:2025aa}}\hskip0pt%DIFAUXCMD
.
}}
\DIFaddend This gap suggests that constrained optimization could be key to addressing both weight boundedness and input constraints in a unified, theoretically grounded framework, particularly in real-time NAC.
...
\begin{itemize}
    ...
    %DIF <  \item Any convex input constraints can be applied to the proposed CONAC, including input bound constraints, and norm constraints. Moreover, the proposed CONAC can handle any combination of these constraints, which can be useful in practical applications where multiple constraints need to be satisfied simultaneously.
    \item \DIFaddbegin \DIFadd{Convex input constraints can be applied to the proposed CONAC, including input bound constraints and norm constraints. Moreover, the proposed CONAC can handle any combination of these constraints, which can be useful in practical applications where multiple constraints need to be satisfied simultaneously.
    }\item \DIFaddend Since the proposed CONAC approximates the entire ideal control law without any conventional controller, prior knowledge of the system dynamics or the system identification process for nominal conventional controller design can be avoided, which is often difficult and time-consuming to implement in practice. 
...
This study extends our preliminary \DIFdelbegin \DIFdel{work in \mbox{%DIFAUXCMD
\cite{Ryu:2024aa}}\hskip0pt%DIFAUXCMD
, where an inequality constraint }\DIFdelend \DIFaddbegin \DIFadd{works in \mbox{%DIFAUXCMD
\cite{Ryu:2024aa,Ryu:2025aa}}\hskip0pt%DIFAUXCMD
, where inequality constraints }\DIFaddend on the weight \DIFdelbegin \DIFdel{norm was }\DIFdelend \DIFaddbegin \DIFadd{and input norm were }\DIFaddend introduced within a constrained optimization framework to ensure boundedness of the NN weights \DIFaddbegin \DIFadd{and satisfy input norm constraint, respectively}\DIFaddend . 
In addition to the weight \DIFaddbegin \DIFadd{and input norm }\DIFaddend constraints considered in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Ryu:2024aa}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Ryu:2024aa,Ryu:2025aa}}\hskip0pt%DIFAUXCMD
}\DIFaddend , the present work \DIFdelbegin \DIFdel{also incorporates }\DIFdelend \DIFaddbegin \DIFadd{incorporates any convex }\DIFaddend input constraints into the same framework to address the convex input saturation problem. 
Moreover, while the previous study was limited to numerical simulations, the proposed CONAC \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend experimentally validated through real-time implementation on a 2-DOF robotic manipulator.
...
Section \ref{sec:stability} analyzes \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend stability of CONAC.
...
%DIF <  The algorithm of the proposed CONAC is given by Algorithm \ref{alg:CONAC}. 
...
%DIFDELCMD < %%%
\DIFdelend \subsection{Mathematical Preliminaries}\label{sec:sub:math preliminaries}
...
\DIFaddbegin \DIFadd{The compact set $\Omega_{n}$ is defined as the set of all feasible states $\q,\dq$ and filtered errors $\fe$.
}\DIFaddend % Furthermore, the theorem states that the norm of $\idealwth$ is bounded such that $\norm{\idealwth}\le \overline\wth<\infty$.
...
\DIFadd{The DNN input vector ${\q}_n$ is defined as ${\q}_n=(\q^\top,\dq^\top,\fe^\top,1)^\top\in\R^{3n+1}$ to include the current state of the system, the filtered error, and the augmentation term $1$ to account for bias term in the input weight matrix $\wV_0$.
}\DIFaddend The \DIFdelbegin \DIFdel{weights $\wV_i,\ \forall i\in\{0,\cdots,k\}$, are initialized randomly using uniform distribution at the beginning of the control process.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The }\DIFdelend activation function is defined as \DIFdelbegin \DIFdel{$\act_i(\mv x):=\left(\sigma(x_1),\sigma(x_2),\cdots, \sigma(x_{l_{i}}), 1\right)^\top,\ \forall\mv x\in\R^{l_i}$}\DIFdelend \DIFaddbegin \DIFadd{$\act_i(\mv x)=\left(\sigma(x_1),\sigma(x_2),\cdots, \sigma(x_{l_{i}}), 1\right)^\top,\ \forall\mv x\in\R^{l_i}$}\DIFaddend , where $\sigma: \R\to\R$ is a nonlinear function, and the augmentation of $1$ is used to account for bias terms in the weight matrices\DIFdelbegin \DIFdel{. 
}\DIFdelend \DIFaddbegin \DIFadd{, similar to the input vector ${\q}_n$.
...
\DIFaddbegin \DIFadd{In selecting the nonlinear function $\sigma(\cdot)$, the ReLU function \mbox{%DIFAUXCMD
...
In this study, the hyperbolic tangent function $\tanh(\cdot)$ was selected as the nonlinear function, \ie \DIFdelbegin \DIFdel{$\sigma(x) = \tanh(x),\ \forall x\in\R$}\DIFdelend \DIFaddbegin \DIFadd{$\sigma(\cdot) = \tanh(\cdot)$}\DIFaddend , which provides desirable boundedness with $\norm{\sigma(x)}<1$ and $0<\norm{\ddtfrac{\sigma(x)}{x}}\le 1$.
...
\begin{equation}
    ...
    .
\end{equation}
However, this \DIFdelbegin \DIFdel{method }\DIFdelend cannot be realized, since we do not \DIFdelbegin \DIFdel{have }\DIFdelend \DIFaddbegin \DIFadd{know }\DIFaddend the exact system dynamics, \ie $\rbM,\rbVm, \rbF$\DIFdelbegin \DIFdel{and $ \rbG$}\DIFdelend \DIFaddbegin \DIFadd{, and $\rbG$}\DIFaddend .
Additionally, the computational cost of the forward sensitivity method is high, as the number of DNN \DIFdelbegin \DIFdel{'s }\DIFdelend weights is generally large.
...
\begin{equation}
    ...
    -\alpha 
    \left(
        \DIFaddbegin {\DIFaddend \pptfrac{\estNN}
        {\estwth}\DIFaddbegin }\DIFaddend ^\top
        \fe
        +
        \textstyle\sum_{j\in\mathcal{I}}
        \lambda_j 
...
\DIFaddbegin \DIFadd{The algorithm of the proposed CONAC is presented by Algorithm \ref{alg:CONAC}, implemented in a discrete-time controller with a sampling time of~$T_s$, where $(\cdot^{(k)})$ denotes the value at the $k$th time step.
}

\begin{algorithm}[!t]
    \caption{\DIFadd{Proposed CONAC Algorithm}}\label{alg:CONAC}
    \begin{algorithmic}[1]
        \STATE \textbf{\DIFadd{Initialize:}} \DIFadd{DNN weights $\estwth^{(0)}$, Lagrange multipliers $\lambda_j^{(0)}$
        }\FOR{$k = 0 $ to $\infty$}
            \STATE \DIFadd{Measure $\q^{(k)},\dq^{(k)}$
            }\STATE \DIFadd{Obtain $\qd^{(k)},\dqd^{(k)}$ from given $\qd(t)$
            }\STATE \DIFadd{$\mv{e}^{(k)}\leftarrow\q^{(k)}-\qd^{(k)}$ and $\ddtt\mv{e}^{(k)}\leftarrow\ddtt\q^{(k)}-\dqd^{(k)}$
            }\STATE \DIFadd{$\fe^{(k)} \leftarrow \ddtt{\mv e^{(k)}}+\mm\Lambda \mv e^{(k)}$ \eqref{eq:err:filtered}
}

            \STATE \textit{\DIFadd{// Neuro-adaptive control law (Section \ref{sec:ctrl design})}}

                \STATE \DIFadd{${\q}_n^{(k)}\leftarrow
                    \left(
                        {\q^{(k)}}^\top,{\dq^{(k)}}^\top,{\fe^{(k)}}^\top,1
                    \right)^\top$
                }

                \STATE \DIFadd{$\rbu^{(k)}\leftarrow\NN({\q}_n^{(k)};\estwth^{(k)}) $ \eqref{eq:control:est}
                }\STATE \DIFadd{Apply $\rbu^{(k)}$ to the system \eqref{eq:sys1}
}

            \STATE \textit{\DIFadd{// Train DNN (Section \ref{sec:adap_laws})}}

            \STATE \DIFadd{Compute $\pptfrac{\NN}{\estwth^{(k)}}$ \eqref{eq:gradient:phi}, $c_j$ and $\pptfrac{c_j}{\estwth^{(k)}}$ for all $j\in\mathcal{I}$          
            }

            \STATE \DIFadd{$\estwth^{(k+1)}
                    \leftarrow
                    \estwth^{(k)}
                    -\alpha 
                    \left(
            ...
            \STATE \DIFadd{$\lambda_j^{(k+1)}
            \leftarrow
            \lambda_j^{(k)}
            +
            \beta_j c_j\cdot T_s$ for all $j\in\mathcal{I}$ \eqref{eq:adap:lbd} 
            }\STATE \DIFadd{$\lambda_j^{(k+1)} \leftarrow \max(\lambda_j^{(k+1)},0)$ for all $j\in\mathcal{I}$ \eqref{eq:adap:lbd:max} 
}
...
\DIFaddend %  SECTION STABILITY ANALYSIS ==============================
...
Similarly to $c_{\theta_k}$, when \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend input constraint $c_j,\ \forall j\in\mathcal{I}\setminus \{\theta_i\}_{i\in\{0,\cdots,k\}}$, is violated, the corresponding Lagrange multiplier $\lambda_j$ increases sufficiently large, leading to an increase in ${\gamma}_2$.
It makes ${\gamma}_2$ \DIFdelbegin \DIFdel{dominates }\DIFdelend \DIFaddbegin \DIFadd{dominate }\DIFaddend the right-hand side of \eqref{eq:lya2:dot}, which \DIFdelbegin \DIFdel{leads }\DIFdelend \DIFaddbegin \DIFadd{makes }\DIFaddend $\ddtt V_2$ negative definite.
...
Therefore, the boundedness of $\fe$ and $\estwth_k$ is guaranteed by the invariant sets $\Theta_{\fe}^{1}$ and $\Theta_{\theta_k}^{1}$, and the \DIFdelbegin \DIFdel{satisfactions }\DIFdelend \DIFaddbegin \DIFadd{satisfaction }\DIFaddend of $c_j,\ \forall j\in\mathcal{I}\setminus \{\theta_i\}_{i\in\{0,\cdots,k-1\}}$, is ensured.
...
\begin{equation}
    ...
    -\alpha
    \left(
        \DIFaddbegin {\DIFaddend \pptfrac{\estNN}{\estwth_i}\DIFaddbegin }\DIFaddend ^\top
        \fe
        +
...
% ------------------------------------
For all controllers, \ie (C$_1$), (C$_2$), (C$_3$), and (C$_4$), the \DIFdelbegin \DIFdel{input vector to the DNN was defined as ${\q}_n=\left(\q^\top,\dq^\top,\fe^\top,1\right)^\top\in\R^{7}$, where the scalar $1$ was included to account for the bias term in the weight matrix.
All }\DIFdelend DNNs shared the same architecture, consisting of two hidden layers with four nodes each, \ie $k=2$, and $l_0=6, l_1=4, l_2=4$, and $l_3=2$ in \eqref{eq:DNN:architecture}.
The \DIFaddbegin \DIFadd{input vector to the DNNs was defined as ${\q}_n=\left(\q^\top,\dq^\top,\fe^\top,1\right)^\top\in\R^{7}$, where the scalar $1$ was included to account for the bias term in the weight matrix.
The }\DIFaddend adaptation gain was set to $\alpha = 0.2$, and the filtering matrix in \eqref{eq:err:filtered} was chosen as $\mm{\Lambda}=\mydiag(5,15)$.
...
\begin{figure}[!t]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.55\linewidth]{
%DIFDELCMD < 		src/figures/lambda_effect.drawio.pdf
%DIFDELCMD < 		}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.65\linewidth]{
	...
	\DIFaddendFL \caption{
        ...
        The notations \DIFdelbeginFL \DIFdelFL{$(\cdot)'$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$(\cdot^{\prime})$ }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{$(\cdot)''$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$(\cdot^{\prime\prime})$ }\DIFaddendFL denote two distinct cases corresponding to large and small values of the Lagrange multiplier, respectively, \ie \DIFdelbeginFL \DIFdelFL{$\lambda_{j}' > \lambda_{j}''$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\lambda_{j}^{\prime} > \lambda_{j}^{\prime\prime}$}\DIFaddendFL .
        % \vspace{-5mm}
    }
...
This was attributed to the fact that the backpropagated error \DIFdelbegin \DIFdel{being }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend scaled by the \DIFdelbegin \DIFdel{elements' }\DIFdelend gradients of the activation function, which is bounded in $(0,1]$, \ie $\sigma(\cdot)=\tanh(\cdot)$.
...
\DIFaddbegin \subsubsection{\DIFadd{Satisfaction of Karush-Kuhn-Tucker (KKT) conditions}}
\DIFaddend 
...
\DIFaddbegin \DIFadd{Since the proposed CONAC was implemented in discrete time, a direct demonstration of the KKT conditions at steady state is not straightforward. 
}\DIFaddend % % \MSRY{
...
\DIFaddbegin \DIFadd{This is because, the KKT conditions are typically defined for continuous-time systems, and their direct application to discrete-time systems requires careful consideration of the sampling rate.
...
\end{equation}
with the maximum allowable control input magnitude $\overline\tau\in\R_{>0}$. This input norm constraint is also commonly applied in current and torque control problems for electric motors %DIF <  \cite{Choi:2024aa}.
\DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Choi:2024aa}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend % The corresponding Lagrangian multiplier is $\lambda_{u_b}$. 
...
%DIF <  \bibliography{\template/refs}
\DIFdelbegin %DIFDELCMD < \bibliography{\template/refs_abbr}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibliography{\template/refs}
...
% Biography
% ------------------------------------
\DIFdelbegin %DIFDELCMD < \vspace{-10mm}
%DIFDELCMD < %%%
...
\begin{IEEEbiography}[{
    ...
    He is currently \DIFdelbegin \DIFdel{serving as a researcher at the }\DIFdelend \DIFaddbegin \DIFadd{pursuing the Ph.D. degree in the }\DIFaddend Cho Chun Shik Graduate School of Mobility at \DIFdelbegin \DIFdel{KAIST}\DIFdelend \DIFaddbegin \DIFadd{Korea Advanced Institute of Science and Technology (KAIST)}\DIFaddend , South Korea.
    His research interests include \DIFdelbegin \DIFdel{adaptive control, neural-networks, and constrained optimization.
    }%DIFDELCMD < \vspace{-10mm}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{neuro-adaptive control, online optimization, and contraction theory.
    %DIF >  \vspace{-10mm}
