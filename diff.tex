...
%DIF DEL tmp2.tex   Fri Jul 18 17:57:05 2025
%DIF ADD tmp1.tex   Fri Jul 18 17:57:05 2025
...
On the other hands, in \cite{Esfandiari:2014aa,Karason:1994aa,Esfandiari:2015aa}, the auxiliary systems were incorporated into the adaptation law by adding the auxiliary states to the feedback error for learning.
...
By formulating the NAC problem as an unified optimization problem with constraints, it is possible to adapt the NN weights while minimizing an objective function, \eg tracking error, subject to both weight and input constraints, simultaneously.
...
\cite{Ryu:2024aa,Ryu:2025aa}}\hskip0pt%DIFAUXCMD
}\DIFaddend , the present work also incorporates \DIFaddbegin \DIFadd{any convex }\DIFaddend input constraints into the same framework to address the convex input saturation problem. 
...
The }\DIFaddend weights $\wV_i,\ \forall i\in\{0,\cdots,k\}$, are initialized randomly using uniform distribution at the beginning of the control process.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend The activation function is defined as $\act_i(\mv x):=\left(\sigma(x_1),\sigma(x_2),\cdots, \sigma(x_{l_{i}}), 1\right)^\top,\ \forall\mv x\in\R^{l_i}$, where $\sigma: \R\to\R$ is a nonlinear function, and the augmentation of $1$ is used to account for bias terms in the weight matrices\DIFdelbegin \DIFdel{. 
...
Similarly to $c_{\theta_k}$, when a input constraint $c_j,\ \forall j\in\mathcal{I}\setminus \{\theta_i\}_{i\in\{0,\cdots,k\}}$, is violated, the corresponding Lagrange multiplier $\lambda_j$ increases sufficiently large, leading to an increase in ${\gamma}_2$.
...
Therefore, the boundedness of $\fe$ and $\estwth_k$ is guaranteed by the invariant sets $\Theta_{\fe}^{1}$ and $\Theta_{\theta_k}^{1}$, and the satisfactions of $c_j,\ \forall j\in\mathcal{I}\setminus \{\theta_i\}_{i\in\{0,\cdots,k-1\}}$, is ensured.
...
This was attributed to the fact that the backpropagated error being scaled by the elements' gradients of the activation function, which is bounded in $(0,1]$, \ie $\sigma(\cdot)=\tanh(\cdot)$.
...
\DIFaddbegin \DIFadd{This is because, the KKT conditions are typically defined for continuous-time systems, and their direct application to discrete-time systems requires careful consideration of the sampling rate and the dynamics of the system.
