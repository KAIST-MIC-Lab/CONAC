...
% apply reference without warmup
% r_func = @(t) epi_gen1(t);

% apply reference with warmup and unreachable desired state
%     - warmup is required to avoid initial poor control decision
%     - unreachable will be applied after normal reference
%     - the purpose of it is to show the satifaction of KKT at s.s.
r_func = @(t) epi_gen2(t);

end

function [r1, r2] = epi_gen1(t)
    T = 3; % traj. duration
...
function [r1, r2] = epi_gen2(t)
    ...
    elseif t < WARMUP_T + COOL_T + 24 % (24s normal reference duration)
        [r1, r2] = epi_gen1(t-WARMUP_T-COOL_T);
