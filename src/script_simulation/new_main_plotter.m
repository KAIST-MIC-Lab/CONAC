...
%% 
ctrl_info = {
    struct("path", "5-Apr-2026_18-39-48", "color", "blue", "C_num", 1), ... % CoNAC
    struct("path", "5-Apr-2026_18-45-28", "color", "green", "C_num", 1) ... % CoNAC (large beta)
    struct("path", "5-Apr-2026_18-40-17", "color", "cyan", "C_num", 2), ... % Aux.
    struct("path", "5-Apr-2026_18-40-28", "color", gray, "C_num", 1), ... % CoNAC (0 beta)
    struct("path", "5-Apr-2026_18-40-00", "color", "magenta", "C_num", 1), ... % CoNAC (large alpha)
};
...
plot([warmup_time+ep_time, warmup_time+ep_time], [-5e1 5e1], "Color", "black", "LineWidth", line_width, "LineStyle", "-."); hold on
plot([warmup_time+2*ep_time, warmup_time+2*ep_time], [-5e1 5e1], "Color", "black", "LineWidth", line_width, "LineStyle", "-."); hold on
...
text(warmup_time + 2*ep_time + .2, -1.8, "Episode 3", "FontSize", font_size, "FontName", 'Times New Roman')
...
% maxVal = max([norm_func(c1_u), norm_func(c2_u), norm_func(c3_u), norm_func(c4_u), norm_func(c5_u)]); 
% minVal = min([norm_func(c1_u), norm_func(c2_u), norm_func(c3_u), norm_func(c4_u), norm_func(c5_u)]);
maxVal = u_ball; minVal = 5;
len = maxVal-minVal; ratio = .3;
ylim([minVal-len*ratio maxVal+len*ratio]);
xlim([warmup_time T])
    ax = gca;
...
%% FIG. 2; INTERESTED TIME INTERVAL
int_start_t = 29.5; int_end_t = 31.5;
int_idx = find(t >= int_start_t & t <= int_end_t);

figure(2); clf;
hF = gcf;
hF.Units = fig_unit;
hF.Position(3:4) = [fig_width, fig_height*2];
...
% maxVal = max([norm_func(c1_u), norm_func(c2_u), norm_func(c3_u), norm_func(c4_u), norm_func(c5_u)]); 
% minVal = min([norm_func(c1_u), norm_func(c2_u), norm_func(c3_u), norm_func(c4_u), norm_func(c5_u)]);
maxVal = 11; minVal = 10.5;
len = maxVal-minVal; ratio = .3;
ylim([minVal-len*ratio maxVal+len*ratio]);
xlim([int_start_t int_end_t])
    ax = gca;
    ax.FontSize = font_size; 
    ax.FontName = 'Times New Roman';

% control input (top-view)
nexttile(2)
int_start_t = 29; int_end_t = 31.5;
int_idx = find(t >= int_start_t & t <= int_end_t);

plot(c5_u(1,int_idx), c5_u(2,int_idx), "Color", color5, "LineWidth", line_width, "LineStyle", "-"); hold on
plot(c4_u(1,int_idx), c4_u(2,int_idx), "Color", color4, "LineWidth", line_width, "LineStyle", "-"); hold on
plot(c3_u(1,int_idx), c3_u(2,int_idx), "Color", color3, "LineWidth", line_width, "LineStyle", "-"); hold on
plot(c2_u(1,int_idx), c2_u(2,int_idx), "Color", color2, "LineWidth", line_width, "LineStyle", "-"); hold on
plot(c1_u(1,int_idx), c1_u(2,int_idx), "Color", color1, "LineWidth", line_width, "LineStyle", "-"); hold on

% ball constraint and u2 constraint
rad = 0:0.01:2*pi;
plot(u_ball*cos(rad), u_ball*sin(rad), "Color", "black", "LineWidth", line_width, "LineStyle", "-."); hold on
plot([-100 100], [u_max2 u_max2], "Color", "black", "LineWidth", line_width, "LineStyle", "-."); hold on
plot([-100 100], [-u_max2 -u_max2], "Color", "black", "LineWidth", line_width, "LineStyle", "-."); hold on
grid on; grid minor;
xlabel('$\tau_1$ / Nm', 'FontSize', font_size, 'Interpreter', 'latex');
ylabel('$\tau_2$ / Nm', 'FontSize', font_size, 'Interpreter', 'latex');
xlim([10 11])
ylim([1.25 3.6])
    ax = gca;
...
semilogy(t, c1_lbd(4,:), "Color", color1, "LineWidth", line_width, "LineStyle", "-", "DisplayName", "$\lambda_{\theta_0}$"); hold on
semilogy(t, c1_lbd(5,:), "Color", color1, "LineWidth", line_width, "LineStyle", "-", "DisplayName", "$\lambda_{\theta_1}$"); hold on
semilogy(t, c1_lbd(6,:), "Color", color1, "LineWidth", line_width, "LineStyle", "-", "DisplayName", "$\lambda_{\theta_2}$"); hold on
