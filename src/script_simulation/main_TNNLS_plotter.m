...
for c_idx = 1:1:4
    fprintf("Computing Control Time\n")
    ctrl = ctrl_results.("ctrl"+string(c_idx));

    ctrl_comp_time = mean(ctrl.comp_control_hist);
    fprintf("ctrl CTRL%d: %E\n", c_idx, ctrl_comp_time);

    train_comp_time = mean(ctrl.comp_train_hist);
    fprintf("trin CTRL%d: %E\n", c_idx, train_comp_time);
end
...
figure(17);clf
plot_comp_time(ctrl_results, group, c_list);
...
function [] = plot_comp_time(ctrl_results, group, c_list)
    tl = tiledlayout(2,1);
    font_size = 16;
    line_width = 2;
    lgd_size = 16;
    ...
    for c_idx = 1:1:length(group)
        ...
        ctrl = ctrl_results.("ctrl"+string(ctrl_idx));
        t = ctrl.t;
        comp_time = ctrl.comp_control_hist;
    
        semilogy(t, comp_time, "color", c_list(c_idx,:), "LineWidth", line_width, "DisplayName", ctrl_name(c_idx)); hold on
    end

    xlabel("$t\ (\rm s)$", "Interpreter", "latex")
    ylabel("Control Computing Time $(\rm s)$", "Interpreter","latex")
    set(gca, 'FontSize', font_size, 'FontName', 'Times New Roman')
    grid on
    lgd = legend;
    lgd.Orientation = 'Horizontal';
    ...
    nexttile(2)
    for c_idx = 2:1:length(group)
        ctrl_idx = group(c_idx);
        
        ctrl = ctrl_results.("ctrl"+string(ctrl_idx));
        t = ctrl.t;
        comp_time = ctrl.comp_train_hist;
    
        semilogy(t, comp_time, "color", c_list(c_idx,:), "LineWidth", line_width, "DisplayName", ctrl_name(c_idx)); hold on
    end

    xlabel("$t\ (\rm s)$", "Interpreter", "latex")
    ylabel("Train Computing Time $(\rm s)$", "Interpreter","latex")
    set(gca, 'FontSize', font_size, 'FontName', 'Times New Roman')
    grid on
