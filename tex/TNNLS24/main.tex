...
\begin{figure*}[!t]
    \centering
        \subfloat[BSC]{\includegraphics[width=0.49\linewidth]{fig1.eps}%
        \label{fig: tracking_CM1}}
    \hfill
        \subfloat[DNN-BSC]{\includegraphics[width=0.49\linewidth]{fig2.eps}%
        \label{fig: tracking_CM2}}
    \vfill
        \subfloat[DNN-BSC-A]{\includegraphics[width=0.49\linewidth]{fig3.eps}%
        \label{fig: tracking_CM3}}
    \hfill
        \subfloat[CoNAC]{\includegraphics[width=0.49\linewidth]{fig4.eps}%
...
\begin{figure*}[!t]
    \centering
        \subfloat[BSC]{\includegraphics[width=0.49\linewidth]{fig5.eps}%
        \label{fig: control_CM1}}
    \hfill
        \subfloat[DNN-BSC]{\includegraphics[width=0.49\linewidth]{fig6.eps}%
        \label{fig: control_CM2}}
    \vfill
        \subfloat[DNN-BSC-A]{\includegraphics[width=0.49\linewidth]{fig7.eps}%
        \label{fig: control_CM3}}
    \hfill
        \subfloat[CoNAC]{\includegraphics[width=0.49\linewidth]{fig8.eps}%
...
\begin{figure*}[!t]
    \centering
        \subfloat[Multipliers of CoNAC]{\includegraphics[width=0.49\linewidth]{fig12.eps}%
        \label{fig: multiplier_CoNAC}}
    \hfill
        \subfloat[Weight norms of DNN-BSC]{\includegraphics[width=0.49\linewidth]{fig9.eps}%
        \label{fig: weight_CM2}}
    \vfill
        \subfloat[Weight norms of DNN-BSC-A]{\includegraphics[width=0.49\linewidth]{fig10.eps}%
        \label{fig: weight_CM3}}
    \hfill
        \subfloat[Weight norms of CoNAC]{\includegraphics[width=0.49\linewidth]{fig11.eps}%
...
% If the Assumption \ref{assum1} holds (i.e., $c(\omega^*)<0,c(\hat\omega)\ge 0$), then $\nabla c_{j,k}^T\hat\theta_k>0$ also implies $\nabla c_{j,k}^T\tilde\theta_k>0$, as demonstrated by the following analysis: 
% \begin{equation}
%     \begin{aligned}
...
% It follows that $\nabla c_{j,k}^T\hat\theta_k>0$ holds for all the constraints introduced in Section \ref{sec:cstr candidates}.
...
%         -K_\zeta \zeta - 
%         \bigg({
%         \vert {\tilde z}^TM_0^{-1}k_\zeta\Delta\tau\vert +0.5k_\zeta^2\Delta \tau^T\Delta \tau
%         \over
%         \Vert\zeta\Vert^2
%         }\bigg)
%         \zeta
...
% To scale the difference of the magnitude between ${\tilde z}$ and $\Delta\tau$, $k_\zeta\in\mathbb R_{>0}$ is used.

% Let the control law be defined as 
% \begin{equation}
%     \tau =-{k_z}M_0{\tilde z}-M_0K_p \zeta -M_0{\tilde q}+C_0{z}+G_0-\hat f+M_0 \dot {z^*}
%     \label{eq: control law CM3}
% \end{equation}
% where $K_p$ denotes the gain matrix.
% Then the system is bounded with the following criteria: ${k_q} > 0, {k_z} >1/2, k_\zeta>1, \mu>1, K_p > 0, K_\zeta >{1\over 2}(K_p^TK_p+I),\dot{\hat\theta } = \text{Proj}_{\Omega}[\alpha\nabla\hat\Phi^T{M^{-1}}^{T} {\tilde z}]$.
% The proof of the boundedness can be readily obtained using the Lyapunov function $V=(1/2){\tilde q}^T{\tilde q}+(1/2){\tilde z}^T{\tilde z}+(1/2\alpha)\tilde\theta^T\tilde\theta + (1/2)\zeta^T\zeta$.
