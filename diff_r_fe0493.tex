...
%DIF DEL tmp2.tex   Mon Jun 23 21:23:48 2025
%DIF ADD tmp1.tex   Mon Jun 23 21:23:48 2025
...
More recently, neuro-adaptive control \DIFaddbegin \DIFadd{(NAC) }\DIFaddend approaches have been introduced to approximate uncertain or unknown system dynamics or entire control laws using neural networks (NNs) \cite{Lewis:1998aa,Farrell:2006aa}. 
...
Therefore, addressing these two key issues—ensuring weight boundedness and satisfying input constraints—is essential for the reliable design of \DIFdelbegin \DIFdel{neuro-adaptive controllers}\DIFdelend \DIFaddbegin \DIFadd{NAC}\DIFaddend . 
...
A common challenge in \DIFdelbegin \DIFdel{neuro-adaptive control }\DIFdelend \DIFaddbegin \DIFadd{NAC }\DIFaddend is maintaining the boundedness of the NN weights to ensure stability. 
...
Although these techniques were effective in ensuring boundedness and avoiding weight divergence, they similarly lacked a formal analysis of the optimality of the adapted weights by biasing the \DIFdelbegin \DIFdel{adaptation law }\DIFdelend \DIFaddbegin \DIFadd{weights }\DIFaddend toward the origin \cite{Ryu:2024aa}.
...
This issue becomes more pronounced in \DIFdelbegin \DIFdel{neuro-adaptive control }\DIFdelend \DIFaddbegin \DIFadd{NAC }\DIFaddend architectures where NNs are used to augment conventional model-based controllers—such as feedback linearization or backstepping—rather than acting as the sole control input. 
...
To address the aforementioned issue, several approaches have been proposed in \cite{Gao:2006aa,Arefinia:2020aa,He:2016aa,Peng:2020aa,Esfandiari:2014aa,Karason:1994aa,Esfandiari:2015aa} to handle input constraints in \DIFdelbegin \DIFdel{neuro-adaptive control }\DIFdelend \DIFaddbegin \DIFadd{NAC }\DIFaddend systems.
...
On the other hands, in \cite{Esfandiari:2014aa,Karason:1994aa,Esfandiari:2015aa}, the auxiliary systems \DIFaddbegin \DIFadd{were }\DIFaddend incorporated into the adaptation law by adding the auxiliary states to the feedback error for learning.
...
\DIFdelbegin \DIFdel{From }\DIFdelend \DIFaddbegin \DIFadd{Furthermore, from }\DIFaddend a learning perspective, augmenting conventional control methods with NNs by simply adding NN outputs to existing control inputs can interfere with the adaptation process. 
...
\subsubsection{\DIFdelbegin \DIFdel{Limitations of Existing Approaches and }\DIFdelend Potential of Constrained Optimization}
%DIF >  \subsubsection{Limitations of Existing Approaches and Potential of Constrained Optimization}

\DIFdelbegin \DIFdel{Although both the existing methods for weight boundedness and input constraints have shown effectiveness, they come with significant limitations. 
...
}\DIFdelend %DIF >  \MSRY{각 문단에 이미 문제점을 지적해서 요약을 안해도 괜찮을 것 같습니다.}
%DIF >  \ctxt{red}
%DIF >  {
...
A promising approach to overcoming \DIFdelbegin \DIFdel{these limitations }\DIFdelend \DIFaddbegin \DIFadd{the limitations of the existing methods }\DIFaddend lies in constrained optimization.
By formulating the \DIFdelbegin \DIFdel{neuro-adaptive control }\DIFdelend \DIFaddbegin \DIFadd{NAC }\DIFaddend problem as an unified optimization problem with constraints, it is possible to adapt the NN weights while minimizing an objective function, \eg tracking error, subject to both weight and input constraints, simultaneously.
...
\DIFdelbegin \DIFdel{These methods impose constraints to address issues like gradient vanishing in backpropagation. 
}\DIFdelend %DIF >  These methods impose constraints to address issues like gradient vanishing in backpropagation. 
However, to the best of the authors' knowledge, no prior work has applied constrained optimization to \DIFdelbegin \DIFdel{adaptive control }\DIFdelend \DIFaddbegin \DIFadd{NAC }\DIFaddend systems with real-time weight adaptation. 
This gap suggests that constrained optimization could be key to addressing both weight boundedness and input constraints in a unified, theoretically grounded framework, particularly in real-time \DIFdelbegin \DIFdel{neuro-adaptive control}\DIFdelend \DIFaddbegin \DIFadd{NAC}\DIFaddend .
...
\begin{itemize}
    ...
    \item Since the proposed CONAC approximates the entire ideal control law \DIFdelbegin \DIFdel{with no }\DIFdelend \DIFaddbegin \DIFadd{without any conventional controller, }\DIFaddend prior knowledge of the system dynamics \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{or }\DIFaddend the system identification process for nominal conventional controller design can be avoided, which is often difficult and time-consuming to implement in practice. 
    \item The adaptation laws for DNN weights and Lagrange multipliers are systematically derived to solve the defined problem, using constrained optimization theory. These laws guarantee convergence to the first-order optimality conditions \DIFaddbegin \DIFadd{at steady state}\DIFaddend , specifically the Karush–Kuhn–Tucker (KKT) conditions, as described in \cite[Chap. 12, Thm. 12.1]{Nocedal:2006aa}.
...
This study extends our preliminary work in \cite{Ryu:2024aa}, where an inequality constraint on the weight norm was introduced within a constrained optimization framework to ensure boundedness of the NN weights. 
In addition to the weight constraints considered in \cite{Ryu:2024aa}, the present work also incorporates \DIFdelbegin \DIFdel{control }\DIFdelend input constraints into the same framework to address the convex input saturation problem. 
Moreover, while the previous study was limited to numerical simulations, the proposed CONAC is experimentally validated through real-time implementation on a 2-DOF robotic manipulator.
...
Section \ref{sec:stability} analyzes a stability of CONAC.
...
The control objective is to develop a \DIFdelbegin \DIFdel{neuro-adaptive controller }\DIFdelend \DIFaddbegin \DIFadd{NAC }\DIFaddend that enables $\q$ to track a continuously differentiable desired trajectory $\qd:=\qd(t): \R_{\ge0} \to \R^n$, compensating for the unknown system dynamics while addressing the imposed constraints.
...
The architecture of the proposed CONAC consists of a DNN that functions as a \DIFdelbegin \DIFdel{neuro-adaptive controller}\DIFdelend \DIFaddbegin \DIFadd{NAC}\DIFaddend , and a weight optimizer for the DNN, as illustrated in Fig. \ref{fig:ctrl:diagram}.
...
Section \ref{sec:sub:NAC} introduces the \DIFdelbegin \DIFdel{neuro-adaptive controller}\DIFdelend \DIFaddbegin \DIFadd{NAC}\DIFaddend , followed by the DNN model in Section \ref{sec:sub:NN definition}. 
The constrained optimization-based weight adaptation law is presented in Section \ref{sec:adap_laws}.

\begin{figure}[!t]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.8\linewidth]{src/figures/Controller.drawio.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.99\linewidth]{src/figures/Controller.drawio.pdf}
...
\begin{proof}
    ...
    \end{equation}
    According to \eqref{eq:lem:lya:dot}, \DIFdelbegin \DIFdel{one can conclude }\DIFdelend \DIFaddbegin \DIFadd{it can be concluded }\DIFaddend that $\ddtt V$ is negative definite if $\norm{\mv{x}}>\tfrac{a_2}{a_1}$ holds.
...
\begin{equation}
    ...
    \label{eq:lya1:dot2}
\end{equation}
\DIFdelbegin \DIFdel{It is concluded that the filtered error can be stabilized
}\DIFdelend Therefore, it is concluded that the filtered error $\fe$ can be stabilized by adapting $\estwth$ to $\idealwth$, \ie $\hat\NN\to\idealNN$, neglecting the effect of input saturation.
...
\begin{figure}[t]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.8\linewidth]{src/figures/DNN.drawio.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.99\linewidth]{src/figures/DNN.drawio.pdf}
    \DIFaddendFL \caption{
...
The DNN architecture \DIFdelbegin \DIFdel{$\NN({\q}_n;\wth) := \NN_k$ }\DIFdelend \DIFaddbegin \DIFadd{$\NN({\q}_n;\wth) = \NN_k$ }\DIFaddend can be recursively represented\DIFdelbegin \DIFdel{from the $0$th input layer to the $k$th output layer}\DIFdelend , as follows:
\begin{equation}
    \NN_i :=
    \begin{cases}
...
The weights \DIFdelbegin \DIFdel{$\wV_i,\forall i\in\{0,\cdots,k\}$ }\DIFdelend \DIFaddbegin \DIFadd{$\wV_i,\ \forall i\in\{0,\cdots,k\}$, }\DIFaddend are initialized randomly using uniform distribution at the beginning of the control process.
...
\DIFdelbegin \DIFdel{In selecting the nonlinear function $\sigma(\cdot)$, the ReLU function \mbox{%DIFAUXCMD
\cite{Maas:2013aa} }\hskip0pt%DIFAUXCMD
is one of most widely used choices for large DNNs, since it effectively mitigates the gradient vanishing problem during error backpropagation. 
...
In }\DIFdelend %DIF >  \MSRY{생략 가능할 것 같습니다.}
%DIF >  \ctxt{red}
%DIF >  {
%DIF >      In selecting the nonlinear function $\sigma(\cdot)$, the ReLU function \cite{Maas:2013aa} is one of most widely used choices for large DNNs, since it effectively mitigates the gradient vanishing problem during error backpropagation. 
%DIF >      However, for control applications, where relatively shallow DNNs are typically sufficient and the gradient vanishing issue is less severe, the sigmoid function or the hyperbolic tangent function is commonly used as the nonlinear function $\sigma(\cdot)$. 
%DIF >      These functions simplify stability analysis, since they are continuously differentiable and their outputs and gradients are bounded. 
%DIF >  }
\DIFaddbegin \DIFadd{In }\DIFaddend this study, the hyperbolic tangent function $\tanh(\cdot)$ was selected as the nonlinear function, \ie $\sigma(x) = \tanh(x),\ \forall x\in\R$, which provides desirable boundedness with $\norm{\sigma(x)}<1$ and \DIFdelbegin \DIFdel{$\norm{\ddtfrac{\sigma(x)}{x}}\le 1$}\DIFdelend \DIFaddbegin \DIFadd{$0<\norm{\ddtfrac{\sigma(x)}{x}}\le 1$}\DIFaddend .

For simplicity, each layer's weights are vectorized as \DIFdelbegin \DIFdel{$\wth_i:=\myvec(\wV_i)\in\R^{\Xi_i},\forall i\in\{0,\cdots,k\}$}\DIFdelend \DIFaddbegin \DIFadd{$\wth_i:=\myvec(\wV_i)\in\R^{\Xi_i},\ \forall i\in\{0,\cdots,k\}$}\DIFaddend , where $\Xi_i:= (l_i+1)l_{i+1}$ is the number of weights in the $i$th layer. 
The total weight vector $\wth\in\R^{\Xi}$ is defined by augmenting $\wth_i, \forall i\in \{0,\cdots,k\}$\DIFaddbegin \DIFadd{, }\DIFaddend as 
\begin{equation}
    \wth := 
    \begin{pmatrix}
        \wth_k\\
    ...
    \end{pmatrix},
\end{equation}
where \DIFdelbegin \DIFdel{$\Xi={\sum_{i=0}^{k} \Xi_i}$ }\DIFdelend \DIFaddbegin \DIFadd{$\Xi:={\sum_{i=0}^{k} \Xi_i}$ }\DIFaddend represents the total number of weights in the DNN.
...
Define \DIFdelbegin \DIFdel{$\idealact_i:=\act_i(\idealNN_{i-1})$ and ${\idealact}'_i:= \ddtfrac{\idealact_i}{\idealNN_{i-1}}$}\DIFdelend \DIFaddbegin \DIFadd{$\idealact_i=\act_i(\idealNN_{i-1})$ and ${\idealact}'_i= \ddtfrac{\idealact_i}{\idealNN_{i-1}}$}\DIFaddend . 
Similarly, let $\estNN_i$ denotes the output of the $i$th layer with the estimated weight vector $\estwth$, and \DIFdelbegin \DIFdel{$\estact_i:=\act_i(\estNN_{i-1})$ and define $\estact_i':= \ddtfrac{\estact_i}{\estNN_{i-1}}$}\DIFdelend \DIFaddbegin \DIFadd{define $\estact_i=\act_i(\estNN_{i-1})$ and $\estact_i'= \ddtfrac{\estact_i}{\estNN_{i-1}}$}\DIFaddend .
...
\begin{equation}
    \DIFdelbegin %DIFDELCMD < \begin{matrix}
...
\DIFdelend \DIFaddbegin \begin{matrix}
        ...
        \end{aligned}
    \end{matrix}\DIFaddend 
    \label{eq:opt:problem1}
\end{equation}
where $J(\fe;\estwth)$ is the objective function.
Inequality constraints $c_j:=c_j(\estwth),\ \forall j\in\mathcal{I}$, are imposed during the weight adaptation process\DIFdelbegin \DIFdel{to minimize the objective function}\DIFdelend , where $\mathcal I$ denotes the set of the imposed inequality constraints. 
...
The Lagrangian function \DIFaddbegin \DIFadd{$L(\cdot)$ }\DIFaddend is defined as
\begin{equation}
    L(
    ...
    \label{eq:lagrangian}
\end{equation}
where \DIFdelbegin \DIFdel{$\lambda_j,\forall j\in\mathcal{I}$ }\DIFdelend \DIFaddbegin \DIFadd{$\lambda_j,\ \forall j\in\mathcal{I}$, }\DIFaddend denotes the Lagrange multiplier for each constraint.
...
When a constraint $c_j$ becomes active, \ie $c_j > 0$, the corresponding Lagrange multiplier $\lambda_j$ increases\DIFaddbegin \DIFadd{, see \eqref{eq:adap:lbd}, }\DIFaddend to address the violation, \DIFdelbegin %DIFDELCMD < \ie %%%
\DIFdel{see \eqref{eq:adap:lbd}}\DIFdelend \DIFaddbegin \DIFadd{see \eqref{eq:adap:th}}\DIFaddend .
Once the violation is resolved and the constraint is no longer active, \ie $c_j \le 0$, the Lagrange multiplier $\lambda_j$ decreases gradually until it returns to zero\DIFdelbegin \DIFdel{.
Note that this adaption law is similar to the ALM in \mbox{%DIFAUXCMD
\cite{Nocedal:2006aa}}\hskip0pt%DIFAUXCMD
, where the adaptation law for Lagrange multipliers is given by $\lambda_j\leftarrow \max\left(\lambda_j+\tfrac{c_j}{\mu},0\right)$, with $\mu\in\R_{>0}$ being the penalty parameter.
}\DIFdelend \DIFaddbegin \DIFadd{, see \eqref{eq:adap:lbd:max}.
...
$\DIFaddbegin \DIFadd{, 
}\DIFaddend assuming known control directions.
...
This section introduces essential constraints required to ensure the stability of the proposed CONAC, as well as conditions for potential \DIFdelbegin \DIFdel{control }\DIFdelend input constraints.

First, weight norm constraints \DIFdelbegin \DIFdel{$\mv{c}_{\theta}(\estwth):= [c_{\theta_i}(\estwth)]_{i\in\{0,\cdots ,k\}}\in\mathbb R^{k+1}$ }\DIFdelend \DIFaddbegin \DIFadd{$c_{\theta_i}(\estwth),\ \forall i\in\{0,\cdots ,k\}$, }\DIFaddend defined in \eqref{eq:cstr:weight:ball}, are essential to prevent the weights from diverging during the adaptation process\DIFdelbegin \DIFdel{.
...
The gradient of \DIFdelbegin \DIFdel{$\mv{c}_\theta:=\mv c_{\theta}(\estwth)$ }\DIFdelend \DIFaddbegin \DIFadd{$c_{\theta_i}$ }\DIFaddend with respect to $\estwth$ can be obtained by accumulating the gradients of each layer as follows:
\begin{equation}
...
Next, we present the following assumptions, which specify the conditions for the potential \DIFdelbegin \DIFdel{control }\DIFdelend input constraints introduced in Appendix \ref{sec:appen:cstr}:

\begin{assum}
    The constraint function $c_j(\estwth)$ is convex in the \DIFaddbegin \DIFadd{(input-) }\DIFaddend $\tau$-space and satisfies $c_j(0) \le 0$ and $c_j(\idealwth)\le 0$.
    \label{assum:convex}
\end{assum}
...
\begin{lem} 
    If \DIFdelbegin \DIFdel{control input constraint $c_j(\estwth),\ \forall j\in\mathcal I \setminus \{\wth_i\}_{i\in\{0,\cdots,k\}}$ }\DIFdelend \DIFaddbegin \DIFadd{input constraint $c_j(\estwth),\ \forall j\in\mathcal I \setminus \{\theta_i\}_{i\in\{0,\cdots,k\}}$, }\DIFaddend satisfies Assumption \ref{assum:convex}, then $\norm{\pptfrac{c_j}{\estwth_i}},\ \forall i\in\{k-1,\cdots, 0\}$, is bounded, provided the norms of $\estwth_i,\ \forall i\in\{k,\cdots, i+1\}$, remain bounded.
...
The \DIFdelbegin \DIFdel{derivative of $c_j,\ \forall j\in\mathcal I \setminus \{\wth_i\}_{i\in\{0,\cdots,k\}}$}\DIFdelend \DIFaddbegin \DIFadd{gradient of $c_j,\ \forall j\in\mathcal I \setminus \{\theta_i\}_{i\in\{0,\cdots,k\}}$}\DIFaddend , with respect to $\estwth_i$ is represented as
\begin{equation}
...
%DIFDELCMD <     }%%%
\DIFdel{_{=\mm{I}_n}
    }\DIFdelend \DIFaddbegin \pptfrac{\rbu}{\estNN} 
    \DIFaddend \pptfrac{\estNN}{\estwth_i}
    = 
    \pptfrac{c_j}{\rbu} 
    \DIFaddbegin \mm{I}\DIFadd{_n
    }\DIFaddend \pptfrac{\estNN}{\estwth_i}
    .
\end{equation}
...
The \DIFdelbegin \DIFdel{second }\DIFdelend \DIFaddbegin \DIFadd{third }\DIFaddend term, $\pptfrac{\estNN}{\estwth_i}$ is bounded, provided that the norms of $\estwth_i,\ \forall i \in\{k,\cdots,i+1\}$, are bounded. 
This can be verified by using the definition of $\pptfrac{\estNN}{\estwth_i}$ given in \eqref{eq:NN:grad}.
Consequently, the boundedness of \DIFdelbegin \DIFdel{$\norm{\pptfrac{c_j}{\estwth_i}},\ \forall j\in\mathcal I \setminus \{\wth_i\}_{i\in\{0,\cdots,k\}}$ }\DIFdelend \DIFaddbegin \DIFadd{$\norm{\pptfrac{c_j}{\estwth_i}},\ \forall j\in\mathcal I \setminus \{\theta_i\}_{i\in\{0,\cdots,k\}}$, }\DIFaddend depends on the boundedness of $\estwth_i,\ \forall i\in \{k,\cdots,i+1\}$.
...
\begin{theorem}
    For the dynamical system described in \eqref{eq:sys1}, the \DIFdelbegin \DIFdel{neuro-adaptive controller }\DIFdelend \DIFaddbegin \DIFadd{NAC }\DIFaddend in \eqref{eq:control:est} with the weight adaptation laws in \eqref{eq:adaptation law} ensure the boundedness of the filtered error $\fe$ and the weight estimate $\estwth$, under the \DIFdelbegin \DIFdel{control }\DIFdelend input constraints satisfying Assumption \ref{assum:convex} and \ref{assum:LICQ}\DIFdelbegin \DIFdel{.
    }%DIFDELCMD < \ctxt{red}{
%DIFDELCMD <         This holds under the weight norm constraints \eqref{eq:cstr:weight:ball}.
%DIFDELCMD <     }
%DIFDELCMD <     \KHCH{
%DIFDELCMD <         이렇게 표현하면 마치 이 constraints만 있다고 받아들일 수도 있습니다. 꼭 이 표현을 써야할 이유가 있을까요? 없으면 빼주시고 있으면 constraint input constraints도 추가하면 좋겠습니다.
%DIFDELCMD <     }
%DIFDELCMD <     \MSRY{
%DIFDELCMD <         필요조건으로서 weight norm constraint가 필요하여서 this holds ...로 표했습니다. 만일 교수님이 말씀하신데로 혼동이 될 수 있다면, ..., provided that the weight norm constraints \eqref{eq:cstr:weight:ball} are imposed. 로 바꿀 필요가 있을 것으로 보입니다.
%DIFDELCMD <     }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{, provided that the weight norm constraints \eqref{eq:cstr:weight:ball} are imposed.
    %DIF >  \ctxt{red}{
    %DIF >      This holds under the weight norm constraints \eqref{eq:cstr:weight:ball}.
    %DIF >  }
    %DIF >  \KHCH{
    %DIF >      이렇게 표현하면 마치 이 constraints만 있다고 받아들일 수도 있습니다. 꼭 이 표현을 써야할 이유가 있을까요? 없으면 빼주시고 있으면 constraint input constraints도 추가하면 좋겠습니다.
    %DIF >  }
    %DIF >  \MSRY{
    %DIF >      필요조건으로서 weight norm constraint가 필요하여서 this holds ...로 표했습니다. 만일 교수님이 말씀하신데로 혼동이 될 수 있다면, ..., provided that the weight norm constraints \eqref{eq:cstr:weight:ball} are imposed. 로 바꿀 필요가 있을 것으로 보입니다.
    %DIF >  }
...
Invoking the fact that the amplitude of the control input depends only on $\estwth_k$, \ie \DIFdelbegin \DIFdel{see \eqref{eq:control:est}, and \eqref{eq:DNN:architecture} and $\norm{\estact_k}<\infty$}\DIFdelend \DIFaddbegin \DIFadd{$\rbu=\estNN=(\mm I_{l_{k+1}}\otimes \estact_k^\top)\estwth_k$ and $\norm{\estact_k}$ is bounded}\DIFaddend , 
the boundedness of inner layers' weights will be established after proving the boundedness of $\fe$ and $\estwth_k$.
In addition, without loss of generality, weight norm constraints $c_{\theta_i},\ \forall i\in\{0,\cdots,k\}$\DIFaddbegin \DIFadd{, }\DIFaddend are supposed to be active.
This assumption is justified because, even if $c_{\theta_i}$ is inactive, $\estwth$ is adapted to minimize the objective function $J$ as long as the constraint is \DIFdelbegin \DIFdel{not violated}\DIFdelend \DIFaddbegin \DIFadd{inactive}\DIFaddend .
...
%DIFDELCMD <                 j\in\mathcal{I}\setminus \{\wth_i\}_{i\in\{0,\cdots,k\}}
%DIFDELCMD <             }
%DIFDELCMD <             \lambda_j 
%DIFDELCMD <             \pptfrac{c_j}{\estwth_k}
%DIFDELCMD <         \Big)
...
The satisfaction of the constraints\DIFaddbegin \DIFadd{, }\ie \DIFadd{$c_j,\ \forall j\in\mathcal{I}\setminus \{\theta_i\}_{i\in\{0,\cdots,k-1\}}$, }\DIFaddend can be verified through the boundedness of $\estwth_k$.
For the \DIFaddbegin \DIFadd{output layer's }\DIFaddend weight norm constraint $c_{\theta_k}$, if the corresponding Lagrange multiplier $\lambda_{\theta_k}$ increases sufficiently large due to the constraint violation, $\estwth_k$ approaches \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{toward }\DIFaddend the origin as the invariant set $\Theta_{\theta_k}^{1}$ \DIFdelbegin \DIFdel{toward the origin}\DIFdelend \DIFaddbegin \DIFadd{shrinks to a point}\DIFaddend .
The remaining \DIFdelbegin \DIFdel{control }\DIFdelend input constraints can be validated implicitly through the term ${\gamma}_2$ in \eqref{eq:lya2:dot}.
Similarly to $c_{\theta_k}$, when a \DIFdelbegin \DIFdel{control input constraint $c_j,\ \forall j\in\mathcal{I}\setminus \{\wth_i\}_{i\in\{0,\cdots,k\}}$}\DIFdelend \DIFaddbegin \DIFadd{input constraint $c_j,\ \forall j\in\mathcal{I}\setminus \{\theta_i\}_{i\in\{0,\cdots,k\}}$}\DIFaddend , is violated, the corresponding Lagrange multiplier $\lambda_j$ increases sufficiently large, leading to an increase in ${\gamma}_2$.
...
%DIF <  By Assumption \ref{assum:convex} and Lemma \ref{lem:convex:angle}, the control input constraints can be considered as convex constraint in the $\estwth_k$-space.
...
Therefore, the boundedness of $\fe$ and $\estwth_k$ is guaranteed by the invariant sets $\Theta_{\fe}^{1}$ and $\Theta_{\theta_k}^{1}$, and the satisfactions of \DIFdelbegin \DIFdel{$c_j,\ \forall j\in\mathcal{I}$}\DIFdelend \DIFaddbegin \DIFadd{$c_j,\ \forall j\in\mathcal{I}\setminus \{\theta_i\}_{i\in\{0,\cdots,k-1\}}$}\DIFaddend , is ensured.

\subsection*{Case 2: Control input is not saturated.}

Since the \DIFdelbegin \DIFdel{control }\DIFdelend input constraints are inactive, the saturation function $\mysat(\cdot)$ in \eqref{eq:lya1:dot2} and the \DIFdelbegin \DIFdel{constraint functions $c_j,\ \forall j\in\mathcal I \setminus \{\wth_i\}_{i\in\{0,\cdots,k\}}$}\DIFdelend \DIFaddbegin \DIFadd{input constraint functions $c_j,\ \forall j\in\mathcal I \setminus \{\theta_i\}_{i\in\{0,\cdots,k\}}$}\DIFaddend , in \eqref{eq:lya2:dot} can be neglected.
...
\begin{equation}
    ...
    .
\end{equation}
The boundedness of $\estwth_k$ also can be ensured by the invariant set \DIFdelbegin \DIFdel{$\Theta_{\theta_k}^{2}=\{ \estwth_k \in \R^{\Xi_{k}} \mid \norm{\estwth_k} \le \overline\theta^*_k \}$}\DIFdelend \DIFaddbegin \DIFadd{$\Theta_{\theta_k}^{2}=\{ \estwth_k \in \R^{\Xi_{k}} \mid \norm{\estwth_k} \le \overline\theta_k \}$}\DIFaddend , as $\lambda_{\theta_k}$ remains positive unless the weight norm constraint $c_{\theta_k}$ is satisfied.
...
\begin{equation}
    ...
    \left(
        ...
        +
        \textstyle\sum\DIFdelbegin \DIFdel{_{
...
Moreover, the Lagrange multiplier $\lambda_j$ does not diverge since the \DIFdelbegin \DIFdel{control input constraints }\DIFdelend \DIFaddbegin \DIFadd{input constraints $c_j,\ \forall j\in\mathcal{I}\setminus \{\theta_i\}_{i\in\{0,\cdots,k\}}$ }\DIFaddend are satisfied before divergence can occur.
...
\begin{figure}[t]
    \centering
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.8\linewidth]
%DIFDELCMD <         %%%
...
%DIFDELCMD <         \includegraphics[width=0.45\linewidth]{src/figures/RobotModel.drawio.pdf}
...
\DIFdelendFL \DIFaddbeginFL \subfloat[Two-link robotic manipulator.]{
    ...
    \DIFaddendFL % \hfill
    \DIFdelbeginFL %DIFDELCMD < \subfloat[Control input saturation function.]{
%DIFDELCMD <         \includegraphics[width=0.5\linewidth]{src/figures/saturation.drawio.pdf}
%DIFDELCMD <         \label{fig:robot:sat}
%DIFDELCMD <     }
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[Control input saturation function.]{
...
$
    \mv{q}_d^{(2)}=\left(-\tfrac{1}{4}\pi,\tfrac{1}{4}\pi\right)^\top
$}\DIFaddend , as illustrated in Fig.~\ref{fig:robot:model}.
...
The corresponding \DIFdelbegin \DIFdel{control }\DIFdelend input constraints are mathematically described in Appendix~\ref{sec:appen:cstr} as the input bound constraint \eqref{eq:cstr:input:bound} and the input norm constraint \eqref{eq:cstr:input:norm}, respectively.
...
\begin{table}[t]
    ...
    \hline
        \DIFdelbeginFL %DIFDELCMD < \multirow{3}{*}{(C$_3$)}%%%
\DIFdelendFL \DIFaddbeginFL \multirow{2}{*}{(C$_3$)}\DIFaddendFL & CONAC without \DIFdelbeginFL %DIFDELCMD < & \multirow{3}{*}{ \shortstack[l]{No input constraints\\are imposed.} } \\
%DIFDELCMD <         & %%%
\DIFdelendFL input constraints.& \DIFaddbeginFL \multirow{2}{*}{ \shortstack[l]{No input constraints\\are imposed.} } \DIFaddendFL \\
        & ($\beta_{\overline{\tau}_2}=\beta_{\underline{\tau}_2}=\beta_{\rbu}=0$) & \\
    \hline
        \multirow{2}{*}{(C$_4$)}& NAC with \DIFaddbeginFL \DIFaddFL{auxiliary system. }\DIFaddendFL & $\overline{\tau}_1=-\underline{\tau}_1=10.428$ \\
        & \DIFdelbeginFL \DIFdelFL{auxiliary system. }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(conventional method) }\DIFaddendFL & $\overline{\tau_2}=-\underline{\tau_2}=3.5$. \\    
    \hline
    \end{tabular}
...
% ------------------------------------
\subsubsection*{Controller 1 \normalfont{(C$_1$)}\DIFdelbegin \DIFdel{: }\textit{\DIFdel{CONAC with a large update rate $\beta_j,\forall j\in\mathcal{I}$, for the Lagrange Multipliers}}%DIFAUXCMD
...
%DIFDELCMD <     끝이 :은 subsubsection으로 자동으로 붙었습니다. 앞의 :을 -와 같은 것으로 바꿀까요?
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend 
...
Therefore, controller (C$1$) utilizes the full capability of the actuator\DIFaddbegin \DIFadd{, see \eqref{eq:sat:tau2} and \eqref{eq:sat:norm}}\DIFaddend .
The weight norm constraints $c_{\theta_i}(\cdot),\ \forall i\in\{0,\cdots,k\}$\DIFaddbegin \DIFadd{, }\DIFaddend defined in \eqref{eq:cstr:weight:ball}, were also incorporated into CONAC framework with $\overline\theta_0=\overline\theta_1=\overline\theta_2=6$.
...
For the \DIFdelbegin \DIFdel{control }\DIFdelend input constraints, large update rates were used\DIFdelbegin \DIFdel{to examine their effect: $\beta_{\overline{\tau}2} = \beta{\underline{\tau}_2} = 10^2$ }\DIFdelend \DIFaddbegin \DIFadd{: $\beta_{\overline{\tau}2} = \beta{\underline{\tau}_2} = 100$ }\DIFaddend and $\beta_{\rbu} = 10$, in order to investigate the effect of large update rates.

% ------------------------------------
% Controllers: CONAC with small beta
% ------------------------------------
\subsubsection*{Controller 2 \normalfont{(C$_2$)}\DIFdelbegin \DIFdel{: }\textit{\DIFdel{CONAC with a small update rate $\beta_j,\forall j\in\mathcal{I}$, for the Lagrange Multipliers}}%DIFAUXCMD
...
The proposed CONAC was implemented with the same input \DIFdelbegin \DIFdel{bound }\DIFdelend constraints as in (C$_1$), but with update rates for the \DIFdelbegin \DIFdel{$[\lambda_j]_{j\in\{\overline{\tau}_2,\underline{\tau}_2,\rbu\}}$}\DIFdelend \DIFaddbegin \DIFadd{input constraints }\DIFaddend reduced by a factor of $100$: $\beta_{\overline{\tau}_2}=\beta_{\underline{\tau}_2}=1$ and $\beta_{\rbu}=0.1$.
All other properties and parameters were kept identical to those of (C$_1$).

% ------------------------------------
% Controllers: CONAC without ctrl cstr
% ------------------------------------
\subsubsection*{Controller 3 \normalfont{(C$_3$)}\DIFdelbegin \DIFdel{: }\textit{\DIFdel{CONAC without control input constraints}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{—}\textit{\DIFadd{CONAC without input constraints}}\DIFaddend }

The third controller was identical to (C$1$) and (C$2$), except that the \DIFdelbegin \DIFdel{control }\DIFdelend input constraints were removed to highlight the constraint handling capability of the proposed CONAC.
In this case, the update rates of the Lagrange multipliers associated with the \DIFdelbegin \DIFdel{control }\DIFdelend input constraints, \ie \DIFdelbegin \DIFdel{$\beta_{\overline{\tau}_2},\beta_{\underline{\tau}_2},\beta_{\rbu}$}\DIFdelend \DIFaddbegin \DIFadd{$\beta_{\overline{\tau}_2},\beta_{\underline{\tau}_2}$ and $\beta_{\rbu}$}\DIFaddend , were set to zero, such that the corresponding Lagrange multipliers $[\lambda_j]_{j\in\{\overline{\tau}_2,\underline{\tau}_2,\rbu\}}$ were not updated, see \eqref{eq:adap:lbd}.
All other properties and parameters were kept the same as in (C$_1$) and (C$_2$).
\DIFdelbegin %DIFDELCMD < \MSRY{
%DIFDELCMD <     weight constraint관련된 파라미터는 C1-3까지 모두 같습니다.
%DIFDELCMD <     C2의 control input 관련 beta만 100배 작아졌다는 말 보충하였습니다.
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend %DIF >  \MSRY{
...
% ------------------------------------
\subsubsection*{Controller 4 \normalfont{(C$_4$)}\DIFdelbegin \DIFdel{: }\textit{\DIFdel{NAC with auxiliary system}} %DIFAUXCMD
\DIFdel{(conventional method)}\DIFdelend \DIFaddbegin \DIFadd{—}\textit{\DIFadd{NAC with auxiliary system (conventional method)}}\DIFaddend }
...
This auxiliary state was incorporated into the adaptation law \DIFdelbegin \DIFdel{\eqref{eq:adap:th:approx} }\DIFdelend by replacing the filtered tracking error $\fe$ with $\fe+\mv{\zeta}$.
...
The tracking performance was quantitatively evaluated using the $L_2$-norm of the each joint angle's tracking error, defined as \DIFdelbegin \DIFdel{$\sqrt{\int_0^T\norm{e_i(t)}\,\der t},\forall i\in\{1,2\}$}\DIFdelend \DIFaddbegin \DIFadd{$\sqrt{\int_0^T\norm{e_i(t)}\,\der t},\ \forall i\in\{1,2\}$}\DIFaddend , where $T\in\R_{>0}$ denotes the duration of each episode.
...
This degradation in tracking performance \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend attributed to actuator saturation during those intervals, as observed in Fig.~\ref{fig:ctrl:result:tau:2} and Fig.~\ref{fig:ctrl:result:tau:norm}.

The tracking performance of CONAC with \DIFdelbegin \DIFdel{control }\DIFdelend input constraints—\ie (C$_1$) and (C$_2$)—outperformed the auxiliary system-based NAC (C$_4$) in the second episode, as shown in Table~\ref{tab:sim:L2}.
This improvement \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend attributed to the ability of (C$_1$) and (C$_2$) to fully utilize the actuator's capacity, whereas (C$_4$) was limited by the design of the auxiliary system, which constrained the first joint actuator.
...
Furthermore, once the control saturation was lifted—typically due to changes in the desired trajectory—this resulted in oscillatory behavior, as CONAC's weights had been adapted to generate excessively high control inputs, \DIFdelbegin %DIFDELCMD < \ctxt{red}{ as illustrated in Fig.~\ref{fig:ctrl:result:tau:1}–\ref{fig:ctrl:result:tau:norm}.}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{see Fig.~\ref{fig:ctrl:result:tau:norm}, where the input constraints were lifted at $25.9$ s and $31.9$ s.
}\DIFaddend 
...
\begin{figure}[!t]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.6\linewidth]{
%DIFDELCMD < 		src/figures/lambda_effect.drawio.pdf
%DIFDELCMD < 		}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.55\linewidth]{
...
To further illustrate this effect, Fig.~\ref{fig:lambda_effect} compares the adaptation direction of \DIFaddbegin \DIFadd{the output layer's weight }\DIFaddend $\estwth_2$ under two different magnitudes of $\lambda_j$. 
\DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{Under Assumptions \ref{assum:convex} and \ref{assum:LICQ}, the convexity of $c_j$ in the $\estwth_2$-space (see Lemma~\ref{lem:convex:angle}) is illustrated in Fig.~\ref{fig:lambda_effect}.
The figure }\DIFaddend highlights the influence of the update rate $\beta_j$: a larger $\beta_j$ (as in (C$_1$)) yields a larger $\lambda_j$, resulting in a more aggressive adjustment of $\estwth_2$ via the term $-\lambda_j\pptfrac{c_j}{\estwth_2}$ in \eqref{eq:adap:th}, thereby accelerating constraint satisfaction—though at the cost of oscillatory learning behavior. 
...
%DIFDELCMD < %%%
\DIFdelend These behaviors are clearly observed in Fig.~\ref{fig:ctrl:result:scope:control} and Fig.~\ref{fig:ctrl:result:scope:beta}, where (C$_1$) satisfies the input constraint $c_{\rbu}$ more quickly than (C$_2$), which shows repeated growth and reset of $\lambda_{\rbu}$. 
\DIFdelbegin \DIFdel{Under Assumptions \ref{assum:convex} and \ref{assum:LICQ}, the convexity of $c_j$ in the $\estwth_2$-space (see Lemma~\ref{lem:convex:angle}) further supports this adaptive behavior.
}\DIFdelend 
...
Once the saturation was resolved at around \DIFdelbegin \DIFdel{$31$ }\DIFdelend \DIFaddbegin \DIFadd{$31.2$ }\DIFaddend s (see Fig.~\DIFdelbegin \DIFdel{\ref{fig:ctrl:result:scope:control}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:ctrl:result:tau:1}}\DIFaddend ), the auxiliary state $\zeta_1$ converged to zero due to the stable state matrix $\mm{A}_\zeta$ in the auxiliary system dynamics, given by $\ddtt\mv{\zeta} = \mm{A}_\zeta \mv{\zeta} + \mm{B}_\zeta \Delta\rbu$ with $\Delta\rbu=\mv{0}_{2\times1}$, as shown again in Fig.~\ref{fig:ctrl:result:scope:aux}.

\hfill 

Finally, the handling of the weight norm constraints $c_{\theta_i},\ \forall i\in\{0,1,2\}$, is investigated. 
Among all controllers, the weight norm constraint was only activated for the output layer weights \DIFdelbegin \DIFdel{$c_{\theta_2}$ }\DIFdelend \DIFaddbegin \DIFadd{$\estwth_2$ }\DIFaddend in (C$_3$), as shown in Fig.~\ref{fig:ctrl:result:weight}. 
This is because the weights in (C$_1$), (C$_2$), and (C$_4$) were implicitly suppressed by the \DIFdelbegin \DIFdel{control }\DIFdelend input constraints, which were not applied in (C3).
...
This \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend attributed to the fact that the backpropagated error being scaled by the \DIFdelbegin \DIFdel{activation function's gradient}\DIFdelend \DIFaddbegin \DIFadd{elements' gradients of the activation function}\DIFaddend , which is bounded \DIFdelbegin \DIFdel{between $-1$ and $1$ (see \eqref{eq:NN:grad})}\DIFdelend \DIFaddbegin \DIFadd{in $(0,1]$, }\ie \DIFadd{$\sigma(\cdot)=\tanh(\cdot)$}\DIFaddend .

Additional details on the constraint handling process are provided in Fig.~\ref{fig:ctrl:result:scope:beta}. 
Similar to the case of the \DIFdelbegin \DIFdel{control }\DIFdelend input constraints, the Lagrange multipliers $\lambda_{\theta_2}$ increased \DIFaddbegin \DIFadd{at around $30.4$ s, as shown in Fig.~\ref{fig:ctrl:result:scope:beta}, }\DIFaddend when $\estwth_2$ exceeded the prescribed norm constraint $\overline\theta_2$ \DIFdelbegin \DIFdel{at around $30.4$ s, }\DIFdelend (see Fig.~\ref{fig:ctrl:result:weight}), thereby adjusting the adaptation direction of $\estwth_2$ to mitigate the violation of the constraint $c_{\theta_2}$\DIFdelbegin \DIFdel{, as shown in Fig.
~\ref{fig:ctrl:result:scope:beta}.
}\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend 

%DIF <  In addition, it is worth noting that the control input constraints were repeatedly violated even after the constraint handling mechanisms were applied, as shown in Fig.~\ref{fig:ctrl:result:scope:control}. 
...
\DIFdelbegin \subsubsection{\DIFdel{Satisfaction of Karush-Kuhn-Tucker (KKT) conditions}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend %DIF >  \subsubsection{Satisfaction of Karush-Kuhn-Tucker (KKT) conditions}

\DIFdelbegin \DIFdel{Since the proposed CONAC was implemented in discrete time, a direct demonstration of the KKT conditions at steady state is not straightforward. 
...
}\DIFdelend %DIF >  Since the proposed CONAC was implemented in discrete time, a direct demonstration of the KKT conditions at steady state is not straightforward. 
%DIF >  % \MSRY{
...
%DIF >  % \ctxt{blue}
%DIF >  % {
%DIF >      This is because, the KKT conditions are typically defined for continuous-time systems, and their direct application to discrete-time systems requires careful consideration of the sampling rate and the dynamics of the system.
%DIF >      Although exact satisfaction of the KKT conditions was not observed—due to the lack of a clear steady state in the discrete-time implementation—the control results remain sufficiently close to the optimal behavior implied by the KKT conditions.
%DIF >  % }

%DIF <  reference 자체가 계속 움직이니까 엄밀하게는 steady state를 보장할 수 없습니다. rough 하게는 theta norm이 어느정도 수렴하므로 이 것을 기반으로 주장해볼 수도 있습니다. theta_dot이 평균적으로는 0에 가까운 것 같서요
\DIFdelbegin %DIFDELCMD < 
...
\begin{figure}[t]
    ...
    \caption{
        Computational time \DIFaddbeginFL \DIFaddFL{$T_{\rm{comp}}$ }\DIFaddendFL of (C$_1$) \protect\colorLine{blue}{solid}, (C$_2$) \protect\colorLine{cyan}{solid}, (C$_3$) \protect\colorLine{gray}{solid}, and (C$_4$) \protect\colorLine{magenta}{solid}.
        %DIF >  \vspace{-5mm}
    }
...
This paper proposed a constrained optimization-based neuro-adaptive controller (CONAC) for unknown Euler-Lagrange systems, addressing both weight norm and \DIFaddbegin \DIFadd{convex }\DIFaddend input constraints within a \DIFdelbegin \DIFdel{rigorous }\DIFdelend \DIFaddbegin \DIFadd{unified }\DIFaddend optimization framework. 
The stability of CONAC was analyzed using Lyapunov theory, demonstrating bounded tracking \DIFaddbegin \DIFadd{errors }\DIFaddend and weight estimation \DIFdelbegin \DIFdel{errors }\DIFdelend under real-time adaptation.
...
\end{equation}
with \DIFdelbegin \DIFdel{$\overline\tau\in\R_{>0}$ denoting }\DIFdelend the maximum allowable control input magnitude \DIFaddbegin \DIFadd{$\overline\tau\in\R_{>0}$}\DIFaddend . This input norm constraint is also commonly applied in current and torque control problems for electric motors\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Choi:2024aa}}\hskip0pt%DIFAUXCMD
.
}\DIFdelend %DIF >  \cite{Choi:2024aa}.
% The corresponding Lagrangian multiplier is $\lambda_{u_b}$. 
The \DIFdelbegin \DIFdel{gradients }\DIFdelend \DIFaddbegin \DIFadd{gradient }\DIFaddend of $c_{\rbu}$ with respect to $\estwth$ are given by
\begin{equation}
...
% Biography
% ------------------------------------
\DIFaddbegin \vspace{-10mm}
\DIFaddend \begin{IEEEbiography}[{
